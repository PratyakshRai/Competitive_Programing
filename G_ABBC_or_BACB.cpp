...
/*
  -> Small Observations:
         so we are given a string made of letter a and b 

         we can pick AB and change it to BC 
         we can pick BA and change it to CB 

         what is the maximum no of coin we can get from it 
         ek baat to clear hai 
         ham 

         a a a a a b se sare a extract kar sakte hai 
         b a a a a se sare a extract kar sakte hai 
         a a a a a a b a a a a a a b a a a a ...
         5 b 6 b
         here the integer represents the number of occurence of a 
         if number of digits is equal to no of b's we can get all the a's 
         what is no(b)<no(digits)
         like 5 b 6 
         b 6 b 5 
         okay one thing is sure if it starts with b ,it will consumes all the a's
         what is it does not start with b
         5 b 6 b 8
         then it is equal to the sum of all the largest digits except the smallest one 
        3 b 10 b 2 b 9 b 5  
        
        what about these cases 
         9 b b b 10 b b 12 
         contigious kitna bhi b ho like b b b b 
         effective sirf do b hi rhenge , like 
         b...b 
...
void Chal_Ja_Plz() {
    ...
    if(b>=cnt){
      int ans=0;
      srt(y);
      for(int i=y.size()-1;i>=0;i--)ans+=y[i];
      cout<<ans<<endl;
      return;
    }      int ans=0;
      srt(y);
      for(int i=y.size()-1;i>=1;i--)ans+=y[i];
      cout<<ans<<endl;
      return;
}
