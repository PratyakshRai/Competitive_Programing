...
/*
  -> Small Observations:
         kab hame lcm semi prime milega ? 
         lcm(a,b)
         case 1 : if a and b both are prime ? right ?  
         like p and q 
         case 2 : 
         ek p ho aur dusara p*q ho right  , where both are prime 
         case 3:

         like what are the cases : 
         a and b should be 
         (p,q) (p,pq)  (q,pq)  (1,p) (1,q) (1 , pq) (pq , pq )
         right ? then only it will be a prime 
...
void Chal_Ja_Plz() {
    ...
    int ans=0;
    map<int,int>sem;
   map<int,int>tprime,mp1,mp2,mp3;
   multiset<int>st;
   for(int i=0;i<n;i++){
    if(is__prime[a[i]]){
      tprime[a[i]]++;
   ...
   for(int i=0;i<n;i++){
    if(is__prime[a[i]]){
      ...
      tprime[a[i]]--;
      cnt--;
      if(mp1.find(a[i])!=mp1.end())ans+=mp1[a[i]];
      if(mp2.find(a[i])!=mp2.end())ans+=mp2[a[i]];
      if(mp3.find(a[i])!=mp3.end())ans+=mp3[a[i]];
    //  ans+=fromsemi(a[i],mp1,mp2,mp3);
    }else if(semi(a[i])){
      auto [x,y]=get(a[i]);
      if(x==y){ans+=tprime[x];mp3[x]--;}
      else {
