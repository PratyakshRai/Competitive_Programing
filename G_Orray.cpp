...
/*
  -> Small Observations:
         prefix xor is maximum , right ? 
         to sabse pahale overall prefix nikal ke dekhlo ki khaha kaha one aa rha hai , right ? 
         the thing is i have to consider bit wise things 
...
void Chal_Ja_Plz() {
    ...
    cout<<endl; 
    // vector<vector<int>>b(35);
    // rsrt(a);
    // vector<int>v(n,1);
    // for(int i=34;i>=0;i--){
    //   int t=1LL<<i;
    //   for(int j=0;j<n;j++){
    //     if(t&a[j]&&v[j]){b[i].pb(a[j]);v[j]=0;}
    //   }
    // }
    // for(int i=34;i>=0;i--){
    //   if(b[i].size()>0)cout<<b[i][0]<<" ";
    // }
    // for(int i=34;i>=0;i--){
    //   if(b[i].size()>1)for(int j=1;j<b[i].size();j++)cout<<b[i][j]<<" ";
    // }
