...
/*
  -> Small Observations:
         the one thing is to notice is the position of 1 , right ? 
         because gcd must be >1 for i>=2 , so 1 must be at the postion 1, righT ? 

         if it is a prime i will leave it alone , right ? 

         so how many no do we have , like ? 
         except 1 and all the primes right ? 
         we do not disturb them like they will create  more fixed points if we swap them with others , right ? 


         now the thing is , can we swap all the others so that we dont get fixed points ? but how ? 
         okay lets swap it with the lagest prime factor 
