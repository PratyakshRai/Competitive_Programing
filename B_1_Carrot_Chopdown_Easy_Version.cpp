...
/*
  -> Small Observations:
        okay to dekha hame ek x choose karna hai jissne 
        hame profit ho ,right ?    
...
void Chal_Ja_Plz() {
/*
    ** Cases Ko Generalize karne ki koshish kro
    ** Agar 15 min me idea kaam nhi kar rha ya jade 
       sara test case ban rha hai to 
       APPROACH BADLO
    ** MIN/MAX me ek baar Binary Search sochna agar 
       current idea kaam nhi kar rha to     
*/
int mx = INT_MIN;
        int n,m;cin>>n>>m;
        vi a(n);
...
// for(auto it:mp)cout<<it.ff<<" "<<it.ss<<"||";
set<int>st;
for(int i:a)if(i%2==0&&mp.find(i/2)==mp.end()){mp[i/2]=mp[i];
st.insert(i/2);
}
...
for(int i=1;i<=m;i++){
    int t=0;
    if(mp.find(2*i)!=mp.end()&&st.find(2*i)==st.end())t=mp[2*i];
    mx=max(mx,total+t);
    if(st.find(i)==st.end())
    total-=mp[i];
}
