...
/*
  -> Small Observations:
         so we have a binary array 
         you have to take all the subsequence 
         and take their median 
         and add all the median of all the subsequnce
         is case me k given hai like the length of that 
         subsequnces 

         so what could be the median median can be either 1 or 0 
         is number of occurence of 1 is >=k/2+1
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
      if(a[i]==0)continue;
      int ways=(nCr(i,temp)*nCr(n-1-i,temp))%MOD;
      ans=(ans+ways)%MOD;
    }
//     vi suff(n+1);
//     int cnt=0;
//     int i=0;
//     while(i<n&&a[i]==0){i++;cnt++;}
//     int ans=0;
//     for(int j=n-1;j>=0;j--)suff[j]=suff[j+1]+a[j];
//     for(; i < n; i++) { // Changed boundary
//     if(a[i] == 0) continue; // Skip if it's not a 1

//     int temp = k / 2;
//     if(i + 1 < n) {
//         if(suff[i + 1] < temp) break; // Not enough 1s left, break out
        
//         int ways = (nCr(n - i + cnt - temp, k - temp - 1) * nCr(suff[i + 1], temp)) % MOD;
//         ans = (ans + ways) % MOD;
//     }
// }
