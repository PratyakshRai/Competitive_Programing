...
/*
  -> Small Observations:
         add x to the set , right ? 
         the definition of mex is changed , right? 
         it is asked to find the kth mex 
         which is defined as , the 
         smallest number divisible by k , which is not in the mex right ?  
         how to solve this ? 
         we cant iterate over it , right ? 
         k and x is very large, right 
         it is guarenteed that x is not in the set ,right 





         the thing is mujhe ye batana hai ki k ka kaunsa least multiple set me nhi hai , binna iterate kiye , right? 
...
void Chal_Ja_Plz() {
    ...
    while(q--){
      ...
      if(type=='+'){
       ...
       int cur=x;
       if(lst.find(x)!=lst.end())cur=lst[x];
       while(pre.find(cur)!=pre.end())cur+=x;
       lst[x]=cur;
       cout<<cur<<endl;
      }
    }
