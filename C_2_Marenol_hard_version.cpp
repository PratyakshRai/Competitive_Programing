...
/*
  -> Small Observations:
         bc ye jaruri hai bhosdike ki sabko fist place pe hi la
         unke beech ka relative distance kam karna hai right ? 
         

         move hota to kitne operation lag sakte hia
...
void Chal_Ja_Plz() {
    ...
    int n;cin>>n;
    string a,b;cin>>a>>b;
    set<int>stoe,stoo,stonee,stoneo;
   ...
   return ;
    ...
    int ans=0;
    for(int i=0;i<ae.size();i++)ans+=abs(ae[i]/2-be[i]/2);
    for(int i=0;i<ao.size();i++)ans+=abs(ao[i]/2-bo[i]/2);
    cout<<ans<<endl;
}
