...
/*
  -> Small Observations:
         in any operation , digits ki value bad rhi hai, 
         aur hamara pichhla element aage aa rha hai,righ? 

         so if the array is already sorted , we cant do any thing , right? 

         otherwise we have to figure out the best way 
         like what we can do is to iterate from the back side 
...
void Chal_Ja_Plz() {
    ...
    multiset<int>st;
    for(int i=0;i<n;i++){
      if(a[i]>mi[i])st.insert(a[i]+1);
      if(a[i]==mi[i]){
        if(!st.empty()&&a[i]>*st.begin())st.insert(a[i]+1);
        else cout<<a[i]<<" ";
      }
    }
    for(int i:st)cout<<i<<" ";
