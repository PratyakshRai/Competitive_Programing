...
/*
  -> Small Observations:
         k==1 is always possible ? right ? 
         with the help of the min and max i can swap any number , right ? 
         like kisi bhi number ko ham uske original position par lga sakte hai 
         min / max ki maddad se , right ? 
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
     if(a[i]!=b[i]) ans=min(ans,max(abs(a[i]-mn),abs(a[i]- mx)));
    }
    cout<<ans<<endl;
}
