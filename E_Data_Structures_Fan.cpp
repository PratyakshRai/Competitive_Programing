...
/*
  -> Small Observations:
         so we are given an array and a binary string 
         and we have to process q queries 
         we cant brute forces like n*q 
          
         for each query 

         there can we only two operation 
         1 l r  from l to r reverser the zero bit 

         2 g , give the xor of all the elements of ai 
         where si = g 
         there are two major problems 
         how should i reverse the bit between range l and r 
         second 
         how should i calculate the xor of the selected position 
         i need to observe somtheing here 
         for each querry  i dont want to traverse it ,how should i do it without traversing ? 
...
void Chal_Ja_Plz() {
    ...
    string s;cin>>s;
    s=" "+s;
    vi pr(n+1);for(int i=1;i<=n;i++)pr[i]=pr[i-1]^a[i];
    int x=0,y=0;
    for(int i=1;i<=n;i++){
      if(s[i]=='0')x^=a[i];
      else y^=a[i];
