...
const int MAX_OPS = 65;

unordered_map<long long, long long, custom_hash> memoMin[MAX_OPS][MAX_OPS];
unordered_map<long long, long long, custom_hash> memoMax[MAX_OPS][MAX_OPS];

long long dpmin(long long x, long long n, long long m) {
    if (x == 0) return 0;
    if (n == 0 && m == 0) return x;
    if (x == 1) return (n > 0) ? 0 : 1; 

    if (memoMin[n][m].count(x)) return memoMin[n][m][x];

    long long ans = INF;

    if (m > 0 && x != 1)
        ans = min(ans, dpmin((x + 1) / 2, n, m - 1));

    if (n > 0)
        ans = min(ans, dpmin(x / 2, n - 1, m));

    if (ans == INF) ans = x;

    return memoMin[n][m][x] = ans;
}

long long dpmax(long long x, long long n, long long m) {
    if (x == 0) return 0;
    if (n == 0 && m == 0) return x;
    if (x == 1) return (n > 0) ? 0 : 1; 

    if (memoMax[n][m].count(x)) return memoMax[n][m][x];

    long long ans = -INF;

    if (m > 0 && x != 1)
        ans = max(ans, dpmax((x + 1) / 2, n, m - 1));

    if (n > 0)
        ans = max(ans, dpmax(x / 2, n - 1, m));

    if (ans == -INF) ans = x;

    return memoMax[n][m][x] = ans;
}
...
void Chal_Ja_Plz() {
    ...
    int capped_n = min(n, 62LL);
    int capped_m = min(m, 62LL);
    
    cout << dpmin(x, capped_n, capped_m) << " " << dpmax(x, capped_n, capped_m) << '\n';
}
// ========================================================================
//                             MAIN FUNCTION
// ========================================================================
