...
/*
  -> Small Observations:
         ...
         so we are given , how many times color i ie ri is searched 
         we 
         have to construct the s1 and s2 
         array 
         also given s1 and s2
         bro s1 and s2 is less than 10 , remember , is it pointing to 
         somewhere? 
         how to solve this question ? 
         jo sabse jade baar ayega usko pahale khojne ka try karenge ? 

         sabse pahale weight ke hisaab se sort kar lenge ? right 

         phir agar s1==s2 rha to  desendig me alternate kar denge
         phir agar koi ek dusare se divisible rha to 
         saara usme hi bhar denge  
         last case s1!=s2 and s1 aur s2 koi ek dusare ko divide nhi kar rhe 
         let s1<s2
         then s1 vale vector me s2/s1 elements dalo phir ek elmente
         s2 ka phir wahi seen , see lcm ke time automatic hi s2 par set ho jayega
         right ?
