...
/*
  -> Small Observations:
         so we have to process q querries , rigth ? 

         for a single querry , how to tell the answer in o(1), is this possible ? 

         so we are given two numbers , a and b 
         also we know that there are p+ and q - 
         let say p me se hamara x amount a ko jata hai , to b ko millga p-x 
         similarly 
         q me se y amount a ko jata hai , to b ko millega q-y
         x*a+(p-x)*b-y*a-(q-y)*b==0, rigth ? 
         variable hai 2 , x and y 
         x*a-x*b+p*b  -y*a+y*b -q*b=0
         x(a-b)-y(a-b)=b*q-b*p
         x-y=(b*q-b*p)/(a-b);
         similarly 
         0<=x<=p
