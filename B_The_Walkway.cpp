...
/*
  -> Small Observations:
         so there are n benches , numbered from 1 to n 
         also there are m cookie seller the ith cookie seller is 
         near the sith bench ? rigth 

         standing in the begning of the walk away , pass through all the benches 
         passes the two consecutive benches in 1  minute 
         knapsack with infinite  amount of cookies  
         going to eat cookie from the bag 
         and buy them during the walk 

         eat cooking only near the bench , if follow the rule 

         there is a cookie seller near the bench , buy the cookie and eat it immediately 
         has not eat any cookie , then take a cookie and eat it   

         har d min pe ek cookie kha rhi hai 

         you can remove atleast one cookie seller 

         minimum possible number of cookie eaten after removing excatly on 
         cookie seller 
         aur phir un cookie seller ko remove karne ka option kya kya hai 
         like a1 ya a2 ya a3 remove kro to minimum hoga , so iscase me 
         3 hoga , right? 
         how to sovle this question ;(

         i can calculate the maximum no of cookie eaten right ? 

         okay so let us assume ,  agar  cookie seller nhi hote to 
         kya hota , waise bhi normally hi har d minute par cookie khate 
         par cookie seller ki wajah se extra cookie khane ki tendency 
         bad gyi hai 
         agar do cookie seller ke beech ka distance d se bda hai ya chhota to uda sakte hai use  ,right  


         is question me kuch nhi karna tha  ,bass 
         har seller ke beech ka independent hoyega , 
         yahi pakadna tha , right ? 
...
void Chal_Ja_Plz() {
    ...
    int total =0;
    for(int i=1;i<s.size();i++)total+=(s[i]-s[i-1]-1)/d;
    total +=s.size()-1;
    int mx=-1;
    int cnt=0;
    for(int i=1;i<s.size()-1;i++){
      int kept = ((s[i] - s[i - 1] - 1) / d) + ((s[i + 1] - s[i] - 1) / d) + 1;
        
        int removed = (s[i + 1] - s[i - 1] - 1) / d;
        
        int diff = kept - removed;
