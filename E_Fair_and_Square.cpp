...
/*
  -> Small Observations:
         bhai ek baat to paaki hai , 
         maan le hamne 
         x y z number choose kiya 

         jisme y ek perfect number tha 
         aur hamne x se y , y se z aur z se x 
         ke path me aane wale sare number ko dekha 
         sab do baar aa rhe honge , except that y , 
         so hame pure tree me ye dekhna hai, ki kaha kaha pefect numer hai  aur 
         bass ley say we got a perfect number T
         to T ke left me total kitne elements hait 
         T ke right me total kitne elements hai 
         aur T ke upar totak kitne elements hai 
         inhi se answer aa sakta hai , 
         agar T ke koi child nhi to T phir useless ho yayega , right  
...
void Chal_Ja_Plz() {
    ...
    for(int i=1;i<=n;i++){
      ...
      if(r*r!=a[i])continue;
        vi comps;
        for(int v:adj[i]){
            if(sz[v]<sz[i])comps.pb(sz[v]);
            else comps.pb(n-sz[i]);
        }
