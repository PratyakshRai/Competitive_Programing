  ...
  */
  ...
  /*
    -> Small Observations:
          so the thing is 2ax + b is the slope for the parabola
          and similarly for  the line segment is k , right ,
          if b2-4ac is less than zero,i can choose x axis by putting k=0, right ? 
  ...
  */
  ...
  void Chal_Ja_Plz() {
      ...
      while(m--){
        ...
        if(pos!=INF){
  if((b-line[pos])*(b-line[pos])<4*a*c){
    YES;
    cout<<line[pos]<<endl;
    continue;
  }
  ...
  /*
  Three golden rules: 
  1. Every problem has line solution 
