...
/*
  -> Small Observations:
         can we say , can we create a permutation such that all ci's are prime ? 
         what is the maximum value of ci at a particular ith place 
         like if we take the sum from backward , 
         we will get the maximum p1+p2..pi value, right , so what is that value ? 
...
int find(set<int>&st,int sum   ,int i){
 int req=(sum+i)/i;
 int idx=lower_bound(all(_prime),req)-_prime.begin();
 while(idx<_prime.size()){
  int p=_prime[idx];
  int l=max(1LL,p*i-i+1-sum);
  int r=p*i-sum ;
  if(l>y)break;
