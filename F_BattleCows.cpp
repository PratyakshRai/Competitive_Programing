...
/*
  -> Small Observations:
         ...
         i cant do n^2 , in this problem , right  ? 
         how to solve this problem then ? 
         what the problem is asled, ? 
         for a particular cow , we have to tell 
         how many to them are above it and below it 
         we can do 

         n log n to handle it , now the question is how should i answer the querries  ? right ? 


         like dekho , mai ye to pta kar sakta hu ki 
         agar qurrry nhi hoti to nlog n time complexity hoti 
...
void Chal_Ja_Plz() {
    ...
    while( q--){
     ...
     for(int i=0;i<N;i++){
      ...
      int r=pos*s+s-1;
      int powr=0;
     if (l == 0) powr = prefixor[r] ^ d; 
            else powr = prefixor[r] ^ prefixor[l-1] ^ d;
      ...
      int ep=0;
      if(en*s==0)ep=prefixor[en*s-1+s];
      else ep=prefixor[en*s-1]^prefixor[en*s-1+s];
      bool flag=false;
      if(powr<ep)flag=true;
      else if (powr==ep&&pos%2)flag=true;
      if(flag){
        ans+=s;
      }
     }
