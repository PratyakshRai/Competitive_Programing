...
/*
  -> Small Observations:
         alice moves first , right / 
         select an integer and remove from the sequence   

         if the sum of the integer selected by alice is even 
         then alice wins , otherwise bob wins ? right ? 

         sare number even hua to alice jeet jayega , right 
         aur agar sare odd numbers hue to alice ke pass (n+1)/2 elements hue , agar ke number even hue to alice jeete ga otherwize bob , right ? 
         now the whole sequence can be divided into 
         two categories , evn and od numbers right ? 

         ab alice kab jitega ,jab alice ke paas 
         koi bhi even number ho , ya to even no of odd number ho 

         note number can be negative also , right ? 
         odd + odd =even no matter the sign , right ? 
         similarly even + odd = odd  , no matter the sign , right ? 
         who will win if they play optimally , right  ? 
         alice ke liye number of odd number odd pad jaye to wo haar jayega ? right ? 
         alice ki turn (n+1)/2 aur bob ki turn n/2 , right ? 
