...
/*
  -> Small Observations:
         the thing is agar pair  three ka multiple rha to phir nhi ho payega , right ? 
         pahala 3 consecutive , dusara 
         6*k wale aa jayenge , right ? 
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
      ...
      if(i>0&&s[i]!=s[i-1])rep++;
      else rep=1;
      ans+=dp[i][1]+dp[i][2]-(rep-1)/2;
