...
*/
bool checker (int val, multiset<int>&a){
  if(a.empty())return false;
  if(val<*a.begin())return false;
  if(val == 1){
    auto it = a.find(1);
    if(it != a.end()){
        a.erase(it);
  ...
  bool ans=false;
  if(val%2==0){
        bool first=checker(val/2,a);
        bool second=checker(val/2,a);
        if(first&&second){
          ans=true;
        }
        ...
        int x=val-1;
        x=x/2;
        bool first=checker(x,a);
        bool second=checker(x+1,a);
        if(first&&second){
          ans=true;
        }
      }
