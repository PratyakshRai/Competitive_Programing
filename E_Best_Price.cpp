...
/*
  -> Small Observations:
         ...
         the thing is it might be possible to make some customer leave 
         , right , 

         is the function monotonous ? 
         agar money badayeing ge to obviously hai ki 
         negative review badega , but the thing is negative review girega bhi , right ? 
         if i do binary search on the answer , can i make it somehow 
         monotonous ? 
         the thing is , let say we have 5 people 
         we set some x value
         aur negative review limit is 3 
         on x we got 4 negative review , but if we increase it more
         4 of the people will not buy our tree but the fith one gonna buy it , and it will make profit there , right  ? 
         so we cant do binary search on the answer , then how i will solve this problem ? 
...
void Chal_Ja_Plz() {
    ...
    srt(b);
    for(int curr:c){
      int amirlog=n-(lower_bound(b.begin(),b.end(),curr)-b.begin());
      int achhelog=n-(lower_bound(a.begin(),a.end(),curr)-a.begin());
      int neg=amirlog-achhelog;
      if(neg<=k){
        ans=max(ans,curr*amirlog);
      }
    }
    ...
    // for(int i=0;i<n;i++)a[i]={aa[i],bb[i]};
    // int ans=0;
    // srt(a);
    // // so hamare paas do case hai kisi bhi i ke liye , righ? let say we have infinite k, then for upcomming i what we can do ,we will check if bi is greater than previ+adder or not if yes we will take it otherwise we will se if the otherguy can afford it or not , if yes then previ+adder otherwise only previ, right ? 
    // // so let say hamara k khatam ho gya hai ,what can we do ,either we have to ignore it , or make it with zero negative review , right 
    // int temp=k;
    // int curr=-1;
