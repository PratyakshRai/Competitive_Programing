...
/*
  -> Small Observations:
         okay to sabse pahale frequency match karni chahiye , rigth ? 
         the thing is the problem with the k+1 thing 
         otherwise , 
         i can get the elements from at the distance from k 
         sort them and check if they are equal or not  ? , right 
         but there the thing is we got two options 
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
      if(i+k>=n&&i-k<0)if(s[i]!=r[i]){NO;return;}
    }
    YES;    
}
