...
/*
  -> Small Observations:
         okay so we have some cases , right ;? 
         if 
         t is already not the sequence , we can say its 0 , right
         otherwise we have to see k , right ? 
...
void Chal_Ja_Plz() {
  ...
  for (int i = n - 1; i >= 0; i--) {
    for (int j = 0; j < k; j++) {
      int next = p[i][j];
      if (next == n) ans[i] = 1;
      else ans[i] = min(ans[i], 1 + ans[next + 1]);
    }
  }
