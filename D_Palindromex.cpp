...
int mex(set<int>&st){
  int i=0;
  for(int t:st){if(t!=i){return i;}i++;}
  return i;
}
int f(int l,int r, vector<int>&a,int center){
  set<int>st;
  st.insert(a[center]);
  int L=center-1;
  int R=center+1;
...
int check (int center,vector<int>&a){
  set<int>st;
  st.insert(a[center]);
  int L=center-1;
  int R=center+1;
...
int check2 (int center,vector<int>&a){
  set<int>st;
...
void Chal_Ja_Plz() {
    ...
    int ans=1;
    ans=max(ans,f(0LL,max(z1,z2),a,z1));
    ans=max(ans,f(min(z1,z2),n-1,a,z2));
    int t=abs(z1-z2);
    if(t%2==0){
      ans=max(ans,check((z1+z2)/2,a));
    }else {
      ans=max(ans,check2((z1+z2)/2,a));
    }
    cout<<ans<<endl;
}
