...
double calculate (vector<int>&pref, int i , int j){
  return (double)(pref[j+1]-pref[i])/(j+1-i);
}
bool can(vector<int>&pref, int length, int testavg,int &l, int &r){
  double maxavg=0;
  for(int i=0;i< pref.size()-1;i++){
    for(int j=i-1+length;j<pref.size()-1;j++){
      double avg=calculate(pref, i, j);
      if(avg>maxavg){
        maxavg=avg;
        l=i;
        r=j;
