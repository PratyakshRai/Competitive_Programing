...
/*
  -> Small Observations:
         so hame ek string di jayegi 
         let say s 

         hame length of shortest string k (let ) btani hai 
         such that us k ko concanate kar kar ke ham s bna de
          but the fault tolerance should be atmax 1 \

          what the fuck is this , how to solve this ? 
          what the proble is pointing to 

          let say the length of s is n 
          we will get the divisor for n 
          how we will do binary search on the divisors (bcoz they will only contain our answer ? right ? )
          let say one of the divisor is x 
          so we will check wheather this is possible or not 
          by simple iterating it 
          like 
...
bool can (string s, int n , int k ){
  ...
  for(int i=0;i<k;i++){
    ...
    srt(st);
    for(int i=0;i<st.size()-1;i++)cnt+=st[i];
