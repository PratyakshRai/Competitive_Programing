...
void Chal_Ja_Plz() {
    ...
    int r,g,b;cin>>r>>g>>b;
    vector<pair<int,char>>color={{r,'R'},{g,'G'},{b,'B'}};
    rsrt(color);
    ...
    int s=tot/2;
    for(int i=0;i<s-zcnt;i++){ans+=y;ans+=x;}
    for(int i=0;i<s-ycnt;i++){ans+=z;ans+=x;}
    if (s-ycnt > 0) {
        for (int i = 0; i < s-xcnt; i++) { ans += z; ans += y; }
    } else {
        for (int i = 0; i < s-xcnt; i++) { ans += y; ans += z; }
    }
    cout<<ans<<endl;
