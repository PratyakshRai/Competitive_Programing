...
/*
  -> Small Observations:
         so for a position i , i have to see till where it can make +prefix sum right ? 
         inshort , from a particular point i 
         i have to check how many +ve comes in the prefix array from the i to n
         number those points as smallest ones , right ? 

         if from ith position suppose we get +ve 
         at x and y 
         then i know i have to number x and y smaller than ith position value , 
         but what should i number them ? 
         because from position x there might be condition that 
         at y there will be positive , or negative 
         if there is positive then x should be greater than y 
         so 
         what i can do it first time if no value is dicided 
         at x or y 
         then i can simple write it in accending order 
         but when i will reach at that position 
         ie at x and found out that at y there is some postive value i will simple swap the elements of x and y , right ? 

         now the question is how to get prefix sum and how to find that within the range of that prefix there are some +ve values , right ? 


         // what was the solution 
         maintain prefix array , 
         now the beauti tells us that , add all those pair (i,j)
         ie let x =pre[j-1]-pre[i-1]
         now what i did wrong is that i have to focus on the beauti , right ? 
         so x kab positive hoga, ? 
         jab pre[j-1]bda ho ,it means agar x positive hai to hame 
         jth index par chhota aur ith index par bda rakhna hoga ,, right ? 
         it means for a particular prefix i
         how much it contributes  it 
         that in the actual permutataion 
         how many elements are greater than i that are larger than it - how many element smaller than it at greater index right 
