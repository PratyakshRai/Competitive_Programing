...
/*
  -> Small Observations:
         ...
         ai*aj=j-i; 
         count the number of pair , since the indexes are use , so we can't sort the array ?

         let us pick a index i , so hame sirf age ka dekhenge , otherwize 
         two times count ho jayega, 
         but the thing is ki ham dekhe kaise , 

         agar ai ko fix kar den to hame ai ke multiples aage seach karne hai , right ? 
         like 
         aj=(j-i)/ai
         j-i ki kya value ho sakti hai 1 , 2 , 3, 4 ,5 , 
         it means hame kya khojna hai aage , 
         how many multiples of ai are there after ith index , note the multiple should be less than ai*(n-i), right ? 
         nah nah nah , sirf multiple se kaam nhi chalega, unka index bhi same usi jaga hona chahiye , right? , how should i find that ? 

         , now how should i find that ? 
