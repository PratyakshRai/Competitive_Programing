...
/*
  -> Small Observations:
         how to do this problem , i can getit ? 
         how to choose numbers such that best choosing time can be acieved  ? 
         does it involves median ? 
         how to do that ? 

         we have to tell  : 
         max(min(ai-x1,ai-x2,ai-x3))? 
         what is the miminum value for 
         abs(xi-c) when c belongs to median ? right ? 
         but in that case there is a sum , and we dont want sum ? right ? 
...
*/
bool can(int mid , vector<int>a,int n){
  int r=0;
  int curr=0;
  for(int i=0;i<n;i++){
    if(a[i]>curr){
