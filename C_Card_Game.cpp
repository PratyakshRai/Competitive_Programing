...
/*
  -> Small Observations:
         okay one thing is to notice , n is very small ,right  ? 
         so 
         what can we do  ? there is a modulo also ,right  ? 
         lets see how alex wins ? right ? 

         kisi bhi turn par , samne wale ke paas koi strong card na ho
         
         pahali baar me hi jeet rha ho , ye kab hoga 

         okay first see when it is draw , jab samne wale ke paas
         hamesh ek tod ho 
         kyuki dono optimally khel rhe hai , isiliye 
         draw ka ek hi option hai , 

         agar pahali baar me hi jit rha hai  , to alice ke 
         paas nth card hona chahiye 

         then thing is how should i calculate the diffent ways 
         if alice wins first ? right ? 
         to combination kya ayega 
         n/2*(n/2-1 )right  ? 
         then after (n/2-1)*(n/2-2),right  ? 
         no it is not like that 
         its ,something like
         let alice first 
         then remaining cards at alice side is n/2-1 
         and remaing cards at bob is n/2
         then picking one of them will be n/2-1c1*n/2c1
         + picking two n/2-1c2*n/2c2 and so on 
  -> Your Attacks:
...
void Chal_Ja_Plz() {
    ...
    b[2]=0;
    for(int i=4;i<=n;i+=2){
        a[i]=(nCr(i-1,i/2-1)+b[i-2]);
        b[i]=nCr(i-2,i/2-2)+a[i-2];
        a[i]=a[i]%MOD;
        b[i]=b[i]%MOD;
    }
    cout<<a[n]<<" "<<b[n]<<" 1"<<endl;
