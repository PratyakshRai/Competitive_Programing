  ...
  */
  ...
  /*
    -> Small Observations:
          so p is between l and  r , right ? 
          okay so there are n offers , right ? 

          so for n offers you can say either   
          either ignore the offer or tell ai is >= p or ai is <= p 
          is you win you will get  a + of |ai-p| right ? 
          otherwize you will loose that much , right 

          so what is the definition of my score , right 
          for all the values of p , what is the minimum number i get  
          right ? 
          ek baat to pakki hai , jo 
          ai l se chhota hoga , uske liye to baat clear hai , right ? 
          aur jo ai r se bda ya brabar hoga uske liye bhi rasta clear hai 
          ab baat fas jati hai ,un ai ke liye jo l aur r ke beech me aate hai , right  

          like l aur  r ke jo bahar hai uska clear hai , right ? 

          now think how should i extract the min value , that is 
          also  a problem , right ? 
          the thing is jo midian se bade hai wo to fayda denge hi denge, right ? 
  ...
  */
  ...
  void Chal_Ja_Plz() {
     ...
     vi pre(m+1,0);
     for(int i=0;i<m;i++){
      pre[i+1]=pre[i]+andar[i];
     }
     int mm=-INF;
     for(int k=0;k<=m;k++){
      int sufsm=pre[m]-pre[k];
      int Sl1=bl+(k*l-pre[k])+sufsm-(m-k)*l;
      int Sr1=br+(k*r-pre[k])+sufsm-(m-k)*r;
      mm=max(mm,min(Sl1,Sr1));
      if(k<m){
        int sufsm=pre[m]-pre[k+1];
      int Sl2=bl+(k*l-pre[k])+sufsm-(m-k-1)*l;
      int Sr2=br+(k*r-pre[k])+sufsm-(m-k-1)*r;
       mm=max(mm,min(Sl2,Sr2));
      }
     }
     cout<<mm<<endl;
  }
