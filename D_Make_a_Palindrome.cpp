...
/*
  -> Small Observations:
         the thing is ki k se chhota ho hi nhi sakta , right 
         aur hame steps ko minimize bhi nhi karna , 
         to kyu na ham k-1 length ka palindrome bnane ki koshish kre ? 

         some observation ,
         agar pahale se hi palindrome hai to baat hi kya, right ? 
         if k==1 , the answer is always  ,right ? ie empty array 
         if k==2 the answer is always right ? ie 1 length array , 
         if k==3 length should be 2 right ? ie both the elment should be same 

         if k==n to koi chhara nhi {
         is case sabse pahale , n aur max element ki frequency 
        dono same parity me hone chahiye , right ? 
...
void Chal_Ja_Plz() {
    ...
    int n,k;cin>>n>>k;vi a(n);invec(a);
    if(k==1){
      YES;return;
    }
    int val=0;
    ...
    val=a[k-2]; 
    vi c ;
    for(int i:b)if(i<=val)c.pb(i);
    int l=0,h=c.size()-1;
    int extra=c.size()-k+1;
    while(l<=h){
      if(extra<0){
        NO;return;
      }
