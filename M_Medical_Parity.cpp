...
int memo[1000005][2];
    ...
    int dp(string &s,string &x, int i,int n,int cnt){
      if(i>=n)return 0;
      if(s[i]=='1')cnt++;
    if(memo[i][cnt%2]!=-1)return memo[i][cnt%2];
      if(i==n-1){
        if((cnt%2)==x[i]-'0')return memo[i][cnt%2]=0;
        else return memo[i][cnt%2]=1;
      }
      int ans=INF;
      if((cnt%2)==x[i]-'0'){ans=min(ans,dp(s,x,i+1,n,cnt%2)); 
      ans=min(ans,dp(s,x,i+1,n,((cnt%2)==0)?1:0)+2);
      }
      else {if(cnt%2!=0)ans= min(ans,dp(s,x,i+1,n,0)+1LL);
      else ans=min (ans,dp(s,x,i+1,n,1)+1LL);
      ans=min(ans,dp(s,x,i+1,n,cnt%2)+1);}
      return memo[i][cnt%2]= ans; 
    }
void Chal_Ja_Plz() {
