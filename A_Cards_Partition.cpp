...
/*
  -> Small Observations:
         what is the maximum value i can achive 
         its basically n , right ? 
        can i do binary search here ? 
         lets suppose i made x partion ,
          so the first thing x should be greater 
          than the largest available ai , 
          otherwise there might me some issue , right 
          is there any possiblily that we cant make the answer , ? 
          we should do binary search on the length of partion, right ? 
          i can do binary search ,right ? 
...
*/
  bool can (vector<int>&a,int length , int k){
    int n=a.size();
    int mx=*max_element(all(a));
    int tot=accumulate(a.begin(),a.end(),0LL);
    int t=(tot+length-1)/length;
    int partition =max(mx,t);
    if (partition*length-tot>k)return false;
    int sum =0;
    for(int i=0;i<n;i++){
      if(a[i]<partition)sum+=partition-a[i];
    }
    return (partition*length-tot<=sum);
  }
void Chal_Ja_Plz() {
