...
/*
  -> Small Observations:
         what does l*k+dth means here , right  ? 

         like sabse pahale dith second pe red hoga , phir har kth second me wo repeat karega , right ? 
         the thing is har kth second par repeat hoga , 
         yahi kth second se pahale sab anpa ek pattern bna lenge, right ? 
         kya ham ye kah sakte hai ,ki agar kth second ke baad 

         ek baar last light ko cross kar diya to khel khatam  , right ? 
         the thing is kuchh samay baad chije wapas se repeat hone lag jayengi 
...
void Chal_Ja_Plz() {
    ...
    while(q--){
      int a;cin>>a;
      int curri=lower_bound(p.begin()+1,p.end(),a)-p.begin();
      if(curri>n){
        YES;
        continue;
      }
      int currdir=0;
      int currtime=(p[curri]-a)%k;
      bool left=false;
      vector<vector<int>>path;
      while(true){
        if(visited[curri][currdir][currtime]==true)break;
        visited[curri][currdir][currtime]=true;
        path.pb({curri,currdir,currtime});
        if (currtime == d[curri]) {
                currdir = 1 - currdir; 
            }
        if(currdir==0){
          ...
          int distance=p[curri+1]-p[curri];
          currtime=(currtime+distance)%k;
          curri++;
        }else {
          if(curri==1){
            left=true;
          ...
          int distance=p[curri]-p[curri-1];
          currtime=(currtime+distance)%k;
          curri--;
        }
