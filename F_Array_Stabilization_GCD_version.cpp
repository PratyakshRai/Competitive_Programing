...
*/
bool check (int  k , int n , const vector<vector<int>>&st, const vector<int>lg){
    int exp_gcd=-1;
    for(int i=0;i<n;i++){
        int l=i, r=i+k, len=r-l+1;
        int j=lg[len];
        int curr=gcd(st[j][l],st[j][r-(1<<j)+1]);
        if(exp_gcd==-1)exp_gcd=curr;
        else if (exp_gcd!=curr)return false;
...
void Chal_Ja_Plz() {
    ...
    int mxpw=lg[2*n]+1;
    vector<vector<int>>st(mxpw,vector<int>(2*n,0));
    for(int i=0;i<2*n;i++)st[0][i]=A[i];
    for(int j=1;j<mxpw;j++){
