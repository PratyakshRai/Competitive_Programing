...
void Chal_Ja_Plz() {
    ...
    vector<vector<bool>>abl(n,vector<bool>(n,false));
    for(int i=0;i<n;i++){
      ...
      for(int j=i;j<n;j++){
        ...
        mn=min(a[j],mn);
        mx=max(a[j],mx);
        if(mx-mn==j-i)abl[mn][mx]=true;
      }
    }
    for(int ans=n/2;ans>0;ans--){
      for(int i=0;i+2*ans<=n;i++){
        if(abl[i][i+ans-1]&&abl[i+ans][i+2*ans-1]){
    ...
    //     if(st.size()>len){
    //       auto it =st.find(a[re]);

    //       st.erase(it);
    //       re++;
    //     }
    //      if(st.size()==len) {
    //       if(*st.rbegin()-*st.begin()+1==len){
    //         pos=i+1;
    //         int del=pos;
    //         multiset<int>rt;
    //           for(int j=pos;j<n;j++){
    //             rt.insert(a[j]);
    //             if(rt.size()<len)continue;
    //             if(rt.size()>len){
    //               auto it =rt.find(a[del]);
    //               rt.erase(it);
    //               del++;
    //             }
    //             if(rt.size()==len){
    //               if(*rt.rbegin()-*rt.begin()+1==len){
    //                 if(*st.rbegin()+1==*rt.begin()){
    //                   cout<<len<<endl;
    //                   return;
                    
    //                 }
    //                 if(*rt.rbegin()+1==*st.begin()){
    //                   cout<<len<<endl;;
    //                   return;
    //                 }
    //               }
