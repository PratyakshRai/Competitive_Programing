...
/*
  -> Small Observations:
         okay to phale , array update kar rha hai
         sare naye formed array ko concatenate kar rha hai , right ? 
         aur phir unme distinct element nikal rha hai , rigth ? 
         isi tarah sare do distinct pair le rha hai , aur unka 
         sum le rha hai  
          
         how to sovle this problem ? 


         let say current array a0 hai 

         naya form hua a1 
         to pair (a0,a1) ka answer hamesha n+1 hoga
         like kisibhi do consecutive pair 
         like (ai,ai+1) ka answer n+1 hoga , right ? 

         ab a0 a1 hai 
         naya aaya 
         a2 
         ko a1 aur a2 ka pair ka answer bhi n+1 hoga? 

         but what about the combination of a2 with the previous ones ? 
         like 
         if the current array is ai 
         then how to tell what is the answer for the combination for 
         the previous arrays , like ai with ai-2 till a0 ? 
         
         it is also the thing that Ai Aj can be same , and cant be same 
         what the problme is pointing to , how to think to this shit ? 

         ek baat to clear hai answer hamesha
         >=nC2 ke hoga like n*n*(n-1)/2;
         frequency banayenge, 
         jab bhi koi naya element ayega , ham uski freq bada denge
         usse ye pata chalega ki wo kitne array me aaya hai 

         firt i will have the base array 
         then after the updation 
         i will have the freq of new charectors , like 
         like say we will have 
         1 2 3 (base), 1 5 3 , 1 5 6 

         so the freq of new elemetn is 5:2 , 6:1
         for 5 , each of the elment of 5 will contribute to the 
         non 5 array
         like freq(of 5)*(no of array having no 5 )
...
void Chal_Ja_Plz() {
    ...
    int n, m;cin>>n>>m;
    vi a(n);invec(a);
   vector<int>last(n+m+3,0);
   vector<int>cnt(n+m+3,0);
   for(int i=0;i<n;i++){
    last[a[i]]=0;
   }
   for(int i=1;i<=m;i++){
