...
/*
  -> Small Observations:
         ...
         a list of m pairs 
         like they does not know each other 
         
         any pair not present in the list are friends 

         to ab ek subsequent good kab hai 

         let it starts from A and end at B
         if we pick two guys from A and B and make a pair from them 

         the pair should not belongs to given m pairs , right ? 

         so how many good subsegments are possible ? 

         we can use set theory 
         total segment - bad segments 
         total segment = n*(n+1)/2 

         now the question arrise how to calculate the bad segments , right ?
...
2. Every problem has a impler solution 
