...
/*
  -> Small Observations:
         we can have a lots of number for 2^31 , righ t? 
         so 
         xor of elements at odd indices must be equal ot xor of elements at even indices , right  ? 

         the point is all the numbers should be same , right ? 
         how should i distibutes those numbers ? 

         can we say xor of all the element should be zero , right ? 
...
void Chal_Ja_Plz() {
    ...
    vector<int>ans;
    int y=0;
    for(int i=0;i<t;i++){ans.pb(i);y^=i;}
    for(int i=t+1;ans.size()<n-1;i++){
        y^=i;
        ans.pb(i);
    }
    ans.back()|=(1<<30);
