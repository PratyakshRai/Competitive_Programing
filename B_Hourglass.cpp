...
/*
  -> Small Observations:
         there will be two cases , yatoi k s se chhhota hoga ,yato bda hoga ? right? 
         s k  m is the input , rigth 
         m k se bda hoga nhi to chhota hoga , right ? 
         we will take m%k this will be the extra minute ? right 

         har k minute par flip kar rha hai ,ho m minute se pahale 
         kitna flip hua hoga ? 
         m/k 
         if this is odd then min(k,m) sand is left 
         if this is even t=s-k , min(s,t) 
...
void Chal_Ja_Plz() {
    ...
    if(k>m){
      if(s<m)cout<<0<<endl;
      else cout<<s-m<<endl;return;
    }

    if(h&1){
      int y=min(s,k);
      cout<<max(y-t,0LL)<<endl;;
      return;
    }
    else {
