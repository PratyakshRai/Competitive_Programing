...
/*
  -> Small Observations:
         here , i have given nxn cells 
         and i have to choose n cells such that 
         the set formed by choosing any two disticnt cell , has the maximum size 
         matlab n cell choose kara tumne 
         aur unme se 2 cells ke beech ka difference set  store kiya 
         to wo set ka size maximum hona chahiye, right ? 
         wo tum kya cells choose karoge

  -> Your Attacks:
         
        what could be the possible values for manhattan distance ? 
        both are diagonal opposite right? 
        ,isnt it simple ? 
        
        but the thing is we have to take only n points , right ? 
        diagonal will have n blocks , right ? 

        we can have n+n different distances 
        i think diagonal will have the minimum , right ? 

        ek cheej do clear hai ,question me sare hi distances nikal rha hai
        what will be the range of the distnace : 
        if nxn is the size ,then 
        [ 1 : 2*n-2 ] right ? 

        how to get all the distance bro
