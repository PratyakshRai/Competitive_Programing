...
/*
  -> Small Observations:
         to pahale mai zero wali condition fix karunga , right? 
         then phir 
         mai check karunga ki kahi conflict to nhi n ho rha hai , right  ? 
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
        if(s[i]=='1'){
            ...
            if(i+x<n&&res[i+x]=='1')f1=true;
            if(i-x>=0&&res[i-x]=='1')f2=true;
            if(f1||f2){
