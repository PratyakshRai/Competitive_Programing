...
*/
// ye code gmini ka hai , nice code ;*(
int dx[4] = {1, -1, 0, 0};
int dy[4] = {0, 0, 1, -1};
struct State {
    int cost, x, y, dir, steps;
...
void Chal_Ja_Plz() {
    ...
    vector<vector<vector<vector<int>>>> dist(
        n, vector<vector<vector<int>>>(m, vector<vector<int>>(5, vector<int>(4, INF)))
    );

    priority_queue<State, vector<State>, greater<State>> pq;

    pq.push({0, start.first, start.second, 4, 0});
    dist[start.first][start.second][4][0] = 0;
    ...
    while (!pq.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            if (nx >= 0 && nx < n && ny >= 0 && ny < m && a[nx][ny] != '#') {
                ...
                if (n_cost < dist[nx][ny][k][n_steps]) {
                    dist[nx][ny][k][n_steps] = n_cost;
                    pq.push({n_cost, nx, ny, k, n_steps});
                }
            }
        }
    }
