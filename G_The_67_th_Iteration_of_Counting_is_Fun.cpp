...
/*
  -> Small Observations:
         for a particular value , let say x 
         agar uske left me y hai jo ki uska neighbour hai 
         uska kya matlab banta hai ? let say y is the minimum padosivalue , right ? 

         so the thing is 
        agar mere element t par baithna hai 
        aur uska min neighbour y hai to 

        agar t y se bahut bda hai to 
        iska kya matlab hai ? 
        abhi tk sufficient log nhi baithe hai, right ? 
        to sufficient log kab tk baith jayenge, t-1 tk 
        phir jitne bhi t hai wo baithne ke liye eligible ho jayeinge , right ? to phir isme t ka range kaise aa rha hai ? 




        agar y theek t-1 hai to , phir to sahi hai , 
...
void Chal_Ja_Plz() {
    ...
    vector<int>s(m+1);
    for(int i=0;i<m;i++)s[i+1]=s[i]+cnt[i];
      int ans=1;
      int mmod=676767677;
      for(int i=0;i<n;i++){
        if(a[i]==0)continue;
        int nb=INF;
        if(i>0)nb=min(nb,a[i-1]);
        if(i<n-1)nb=min(nb,a[i+1]);
        if(nb>=a[i]){
          cout<<0<<endl;
          return;
        }
