...
void Chal_Ja_Plz() {
    ...
    vi bestdel(n,0);
    for(int i=0;i<=k;i++){
      int actopen=min(i,(int)open.size());
      int closed=min(k-i,(int)close.size());
      vi currdel(n,0);
      for(int x=0;x<actopen;x++){
        currdel[open[x]]=1;
      }
      for(int y=0;y<closed;y++){
    currdel[close[close.size() - 1 - y]] = 1;
      }
      int matches=0;int curr_open=0;
      for(int idx=0;idx<n;idx++){
        if(currdel[idx])continue;
        if(s[idx]=='(')curr_open++;
        else if(curr_open>0){
          curr_open--;
          matches++;
    ...
    for(int i=0;i<n;i++)cout<<bestdel[i];
    cout<<endl;
}
