...
/*
  -> Small Observations:
         so the thing is , he should start deliverying the pizza from the 
         point A, to all these points Xi and then return to the point B
         the rule is that it should end at the point B  , right ? 
         dp lagega ye to pakka hai,
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
      if(mp.find(x[i])==mp.end()){
        ...
        mp[x[i]]={min(y[i],mp[x[i]].first),max(y[i],mp[x[i]].second)};
      }
    }
    int dptop=0,dpbottom=0;
    int ytop=ay,ybottom=ay;
    for(auto it :mp){
      int miny=it.ss.first;
      int maxy=it.ss.ss;
      // ab agar top pe ja rha hu to , niche se bhi ja sarka hu aur upar se bhi , right 
      int topgya=min(dpbottom+abs(ybottom-miny ),dptop+abs(ytop-miny));
      int newdpbottom=topgya+abs(miny-maxy);
      int bottomgya=min(dpbottom+abs(maxy-ybottom),dptop+abs(maxy-ytop));
      int newdptop=bottomgya+abs(miny-maxy);
      dptop=newdptop;
      dpbottom=newdpbottom;
      ytop=miny;ybottom=maxy;

    }
    int ans=0;
    ans+=abs(bx-ax);
    ans+=min(dptop+abs(ytop-by),dpbottom+abs(ybottom-by));
    cout<<ans<<endl;
}
