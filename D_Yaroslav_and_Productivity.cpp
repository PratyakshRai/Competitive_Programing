...
int fun (vector<int>&a,set<int>&st,int idx, int  symbol,vector<vector<int>>&memo){
  if(idx<1)return 0;
  if(memo[idx][symbol]!=-INF)return memo[idx][symbol];
  int ans=-INF;
  int val=0;
  if(st.find(idx)!=st.end()){
    ...
    ans=max(ans,fun(a,st,idx-1,1-symbol,memo)+val);
  } 
    if(symbol)val=a[idx];
    else val=-a[idx];
    ans=max(ans,fun(a,st,idx-1,symbol,memo)+val);
  
  return memo[idx][symbol]= ans;
}
void Chal_Ja_Plz() {
   ...
   vector<vector<int>> memo(n + 1, vector<int>(2, -INF));
    cout<<fun(a,st,n,1,memo)<<endl;
}
