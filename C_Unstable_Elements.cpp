...
/*
  -> Small Observations:
         so the thhing is either we can increase the length of the array by len(set) or we can decrease it , right ? 
         agar same lenght ke consecutive elements hue to bhi khel khatam hai,right  ?
...
void Chal_Ja_Plz() {
    ...
    int ans=0,remove=0,pre=0;
    for (int it :mt){
      int t=st.size();
      int to_rem=it-1-pre;
      int tot=to_rem*st.size();
      if(k>=(n-remove)&&abs(k-(n-remove))%t==0)ans++;
      else if(k>=(n-remove-tot)&&(k-n+remove+tot)%t==0)ans++;
      remove+=st.size()*(it-pre);
      pre=it;
      st.erase(it);
    }
    cout<<ans<<endl;
