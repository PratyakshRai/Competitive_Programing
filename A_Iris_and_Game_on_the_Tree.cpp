...
/*
  -> Small Observations:
         okay so the very first thing is jinke raste me ? nhi hai , like those leafs having no ? will contribute fixed right ? 

         now lets study the 10 and 01 patern ,right ? 

         lets say we have 

         0?1  ab isme ? ki jagah 1 dalo ya 0 -1 ka effect padega
         ayese number ko second category me daal do 
         similarly for 1?0 , ek me 1 dalo ya 0 , +1 ka effect padega , right ? 

         baki 0_0 aur 1_1 me koi effect nhi padega , right ? 
         to ye sare no to fix ho gye , like isme koi kuch nhi kar sakta hai  , right ? 
         ab kaha kuchh kiya ja sakta hai ? 
         ao dekhte hai  , right ? 
         ab agar ? end me ho to ? 
         0_ ke case me yato mai 0 daalse nullyfy kar sakta hu 
         yato 01 karke -1 add kar sakta hu , right ? 

         similarly for 1_ ke case me mai 1 daal ke nully fy kar sakta hu ya 0 daal ke +1 

         similarly agar ? mark ek dam starting me aag gya like root node me 
         ?0 ?1 
         all those applied to them also 

         so our window should be of size 2 ,right ? 

         what is there is three ? 
         like  
         ???
         and what about if there is two ?  ? in the window 
...
*/
const int MAXN = 2e5 + 5; int deg[MAXN];
...
void Chal_Ja_Plz() {
    ...
    if(s[0]!='?'){
        if(s[0]=='1')cout<<l0+(lq+1)/2;
        else cout<<l1+(lq+1)/2;
    }
    else {
        if(l1!=l0)cout<<max(l1,l0)+(lq)/2;
        else cout<<l1+(lq+(in%2))/2;
    }
    cout<<endl;
