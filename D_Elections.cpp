...
/*
  -> Small Observations:
         jo bhi undecidable voters hai , wo samse chhote index wale ko vote karenge 

         for each i we have to find the minimum no of candidate neeta to be removed for the number i to win the elections , right ? 

         so suppose we are checking for some ith position 
         

         ek baat to clear hai  , ye jo c hai wo ek tarah se a[0] ke hi voters hai , right ? 

         so for a position i 

         usse kuchh age bade honge

         usse kuchh pichhe wale bade honge , maybe right ? agar ayesa nhi hai to zero ,right ? 
         agar aage se kisi ko bhi remove karunga to uska voters suruwat wale ko millega , right ? 
         so the thing is 

         age jo bda hai usko hatadiya 
         unke sare votes ab piichhe ko chhale gye , right? 
         to agar iske baad agar piichhe me koi bhi ek bda ho gya to sabko pelna padega , right ? 
...
void Chal_Ja_Plz() {
    ...
    vi a(n);invec(a);
    a[0]+=c;
    vi prefix(n+1),suffmax(n+1);
    for(int i=0;i<n;i++)prefix[i+1]=prefix[i]+a[i];
    suffmax[n-1]=a[n-1];
    for(int i=n-2;i>=0;i--)suffmax[i]=max(a[i],suffmax[i+1]);
    vi prefixmax(n+1);
    prefixmax[0]=a[0];
    for(int i=1;i<n;i++){
    ...
    for(int  i=0;i<n;i++){
        int pre=0;
        int suff=0;
        if(i>0)pre=prefixmax[i-1];
        suff=suffmax[i+1];
        if(max({a[i],pre,suff})==a[i]&&a[i]!=pre)ans.pb(0);
        else {
            
            if(prefix[i+1]<suff)ans.pb(i+1);
            else ans.pb(i);
