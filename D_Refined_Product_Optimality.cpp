...
/*
  -> Small Observations:
         so i have this conclusion that with smallest ai 
         i should have smalles bi 
         but how should i do it in logn , right ? 
...
int mmod=998244353;
 ...
 void changeit(vector<int>&mm,int idx,int val,int&ans){
  int temp=mm[idx];
  mm[idx]=val;
  ans=ans*binpow(temp,mmod-2,mmod)%mmod;
  ans=(ans*val)%mmod;
 }
void Chal_Ja_Plz() {
    ...
    int ans=1;
    for(int i=0;i<n;i++)mm[i]=min(A[i].ff,B[i].ff);
    for(int i:mm)ans=(ans*i)%mmod;
    cout<<ans<<" ";
    while(q--){
      ...
      if(type ==1){
        ...
        int curr=posA[x];
        pair<int,int>tar={a[x],INF};
        int last=upper_bound(all(A),tar)-A.begin()-1;
        posA[x]=last;
        posA[A[last].ss]=curr;
        swap(A[curr],A[last]);
        a[x]++;
        A[last].ff++;
        int nev=min(A[last].ff,B[last].ff);
        if(mm[last]!=nev){
          changeit(mm,last,nev,ans);
        }
       }else {
        pair<int,int>t={b[x],x};
        int curr=posB[x];
        pair<int,int>tar={b[x],INF};
        int last=upper_bound(all(B),tar)-B.begin()-1;
        posB[x] = last;
            posB[B[last].ss] = curr;
        swap(B[curr],B[last]);
        b[x]++;
        B[last].ff++;
        int nev=min(B[last].ff,A[last].ff);
        if(mm[last]!=nev){
          changeit(mm,last,nev,ans);
        }
       }
