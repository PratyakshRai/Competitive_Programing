...
/*
  -> Small Observations:
         so basically we have range for x 
         which is (a,c]
         and for y is 
         (b,d]

         we have to choose x and y such that 
         x.y % a.b == 0 , right  ? 

         we will try to form the smallest x and y , if that is not possible , then -1 
         otherwise that will be our answer ? right ? 

         okay the thing is we have two ranges, right ? 

         if we pick one range for x , then one will be for y 
         mai kaha phas rha hu 
         mai x aur y ko a and b bnane ki koshish kar rha hu , right ? 
         ab mai nhi janta ki kaun sa range kiske liye rakhu ki mera answer parfect baith jaye  
  -> Your Attacks:
...
//   // agar x ka range  a c hai to ham try karenge x me 2*a ya n*a ya b daalne ka right ? 
//   bool flag=false;
//   if(2*a<=c){
//     flag=true;
//     x=2*a;
