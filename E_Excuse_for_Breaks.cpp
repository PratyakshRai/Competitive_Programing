...
/*
  -> Small Observations:
         ...
         on each iteration 
         if a[i]= 1 adds d in v 
         and increament c 
         else make c zero 

         then 
         for each index in p if any one is equal to c 
         add r[j]to vl 
         if c is equal to n 
         makes c 0 , right ? 
         ...
         see c ek ek kar ke badega, right? 
         so its better to take a lenght equal to n 
         as if greater both [1 1 1 1  ]wala will have the same effect right ? 

         naah bruh 
         c i think max kitna le ja sakta hai 
         jisse hame fayda mille ? 
         

         hame kisse matlab hai ? 

         v se 

         v ko max karna hai ,right ? 

         agar a[i] ko zero karte hai to 


        naah
        cases bnate hai 
         
        mai ek chiij kah sakta hu 

        hame break point dekhna hai , right ?
...
void Chal_Ja_Plz() {
   ...
   vi p(m), r(m);
    ...
    auto g=[&](int x)->int {
        int k=x/n;
        int rem=x%n;
        auto it=upper_bound(all(P),rem)-P.begin()-1;
        int ans=k*R.back();
        if(it>=0)ans+=R[it];
        return ans;
    };
