...
/*
  -> Small Observations:
         so one thing is clear , we have to check the contribution  , right ? 
         like 

         first i will put -1 on all the zero , 
         now do the prefix sum 

         now lets start , 
         fisrt i have to check where there is a zero in the prefix sum , and get the sum of all that index (like agar index 5 hai to n-5+1 add karunga , right   ) aur multiply with the
         ek dum left me mai kya utha sakta hun right ?  
...
void Chal_Ja_Plz() {
    ...
    for(int i=1;i<=n;i++){mp[a[i]].first+=i;mp[a[i]].ss++;}
    vector<int>c=a;
    int ans=0;
    int curr=1;
    int sum=0;
   map<int, pair<int,int>> mp2;
    for(int i=1;i<=n;i++){
      if(mp.find(sum)!=mp.end()){
        int total=mp[sum].ff-mp2[sum].ff;
        int len=mp[sum].ss-mp2[sum].ss;
        int y= 0;
        y=(y+len*n)%MOD;
        y=(y+len-total)%MOD;
        y=(y*i)%MOD;
