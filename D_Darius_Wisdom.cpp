...
/*
  -> Small Observations:
         so either 0 ,1 or 2 hoga, 
         aur hame less than equal to n steps me 
         increasing order me ise set karna hai , 
         right ? 
         
         it is guarantee that atleast one column contains 1 in all the test cases , 
         one observation is 1 is used to move two , right ? 

         the optimal way is to put all the one just behind the 2s ,right 


         ye zero jo 2 ke beech me aa ja rhe hai wo kam bigad de rhe hai , right ? 
         with the help of 1 i can move zero to the last place where there is a 2 , right so what  i will do , if there is some zero or 1 hai 2 ke aage, to mai sare zero ko 2 ke piche pade 1 se swap kar dunga, aur sare 1 ko 2 se (2 ke end se ) 
...
void Chal_Ja_Plz() {
    // There is always a simpler solution for the question 
     ...
     auto swp =[&](int u,int v){
      ...
      if(a[v]==2)s2.erase(v);
      swap(a[u],a[v]);
      if(a[u]==0)s0.insert(u);
      if(a[u]==1)s1.insert(u);
      if(a[u]==2)s2.insert(u);
      if(a[v]==0)s0.insert(v);
      if(a[v]==1)s1.insert(v);
      if(a[v]==2)s2.insert(v);
     };
    

    for(int i=0;i<n;i++){
      if(s0.empty()|| *s0.rbegin() <= i)break;
      if(a[i]==0)continue;
      if(a[i]==1){
        int l=*s0.rbegin();
        swp(i,l);
