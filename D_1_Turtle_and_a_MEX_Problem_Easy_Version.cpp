...
/*
  -> Small Observations:
         so there are n sequences 
         the length of ith sequence is li 
         there is non negative integer x 
         you will perform some operations 
         on each operation x is set to the mex (inwhich x is included ) of selected ith sequence 
         then turtle is asked to give the maximum value for x after performing some operations      
         
         agar ham ye nikal le ki us mex ki sequence use karke ham 
         maximum kitna x create kar sakte hai to ye badiya hoga 
         le say ham us mex ka use karne 
         maximum y tak mex bna pa rhe hai 
         to 0 se y tk to y hoga , phir 
         y se aage ham normal add karenge answer me  means
         ham koi operation nhi karenge 
         

         ham ye kaise nikalenge ki sequence maximum mex kitna degi ? 
         using dfs / bfs ? 
...
void Chal_Ja_Plz() {
    ...
    int y=*max_element(all(mex));
    int t=min(y,m);
    int ans=y*(t+1);
    ans+=((m*(m+1)/2-y*(y+1)/2))>0?(m*(m+1)/2-y*(y+1)/2):0;
    cout<<ans<<endl;
