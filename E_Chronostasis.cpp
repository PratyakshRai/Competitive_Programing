...
void Chal_Ja_Plz() {
    ...
    vi a(n);
    invec(a);
    multiset<int> pos,neg,zero;
    for(int i:a){
        if(i==0)zero.insert(0);
        else if (i>0)pos.insert(i);
        else neg.insert(i);
    }
    vi ans;
    if(pos.empty()){
    ...
    for(int i=0;i<n-1;i++){
       auto it = neg.upper_bound(-ans.back());
        ...
        if (it != neg.end()) {
            ...
            if(!pos.empty()){
                ans.pb(ans.back()+*pos.begin());
                pos.erase(pos.begin());
            }
        }
    }
    if(pos.empty()&&neg.empty()&&zero.empty()){
