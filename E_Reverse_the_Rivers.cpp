...
/*
  -> Small Observations:
         n countries with k regions ,
         i denotes the country ,j donotes the region aij denotes the 
         water in it , 

         new value will be prefix or of all the country will that region ,right ?

         each querry contains  m requirements ? right ,
         its a heavy implementattion problem ,i think , right? 
...
void Chal_Ja_Plz() {
   ...
   vector<vector<int>>b(k,vector<int>(n,0));
    for(int i=0;i<n;i++)for(int j=0;j<k;j++)b[j][i]=a[i][j];
    // for(int i=0;i<k;i++){
    ...
    while(q--){
      ...
      while(m--){
        int l,r;
        int region;cin>>region;
        region--;
        string type;cin>>type;
        int val;cin>>val;
        if(!flag)continue;
        if(type==">"){
          auto it =upper_bound(b[region].begin(),b[region].end(),val);
          if(it==b[region].end()){
            flag=false;continue;
          }
          l=it-b[region].begin();
          ...
          auto it = lower_bound(b[region].begin(),b[region].end(),val);

if (it == b[region].begin()) {
    flag=false;continue;
} else {
    --it;
   r = it - b[region].begin(); 
