...
/*
  -> Small Observations:
         we can divide the whole array into two groups 
         one containing -1 , and other containing the other +ve numbers , right ? 
         ai cannot be zero , remember ,right  ? 
         the thing is -1 wale array se agar mai kuchh nikal sakta hu to wo hai -1 or zero , right ? 
         what does i mean here is 
         from the -1 group i can take either -1 or 0 , right no more no less right ? 
...
void Chal_Ja_Plz() {
    ...
    int pos=n-c;;
    int dis=mp.size();
    int c_even=binpow(2,c-1);
    int c_odd= (c==0)?0:c_even;
    int even_poslo=binpow(2,pos-dis);
    int ans=(c_even*even_poslo)%MOD;
    int f=0;
    for(auto it:mp){
      if(mp.count(it.ff+1))f++;
    }
    ans=(ans+((c_odd*even_poslo)%MOD)*f)%MOD;
    cout<<ans%MOD<<endl;
}
