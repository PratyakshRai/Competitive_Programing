...
/*
  -> Small Observations:
         so the thing is ,its not about the minimum 
         its about the minimum + availability , right  ? 
         so i think,it can be done by graph ,  
          
         i am not sure but lets try , we can make two variables, 
         the max , like max kitna hai abhi ka aur abhi tk mimimum 
         kitne me pahucha hai, right ? 
...
*/
  ...
  bool can(map<int,vector<pair<int,int>>>&mp,vector<int>&b,int mx){
    int n=b.size()-1;
    vector<int>dp(n+1,-1);
    dp[1]=b[1];
    for(int i=1;i<=n;i++){
      ...
      for(auto it :mp[i]){
        ...
        int w=it.ss;
        if(w<=mx&&w<=dp[i]){
        dp[nxt]=max(dp[nxt],(dp[i]+b[nxt]));}
      }
    }
    return dp[n]!=-1;
  }
  // int fun(int mx,vector<int>&req,map<int,vector<pair<int,int>>>&mp,vector<int>&b,int idx){
  //   req[1]=0;
  //   int n=b.size()-1;
  //   if(idx==n)return req[n];
  //   if(idx!=n)mx+=b[idx];
  //   int ans=INF;
  //  for(auto p:mp[idx]){
  //   int nxt=p.ff;
  ...
  //   req[nxt]=min(req[nxt],max(w,req[idx]));
  //   ans=min(ans,fun(mx,req,mp,b,nxt));
  //   req[nxt]=old;
  //  }
  //   return ans;
  // }
