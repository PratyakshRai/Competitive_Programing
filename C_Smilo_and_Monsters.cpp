...
/*
  -> Small Observations:
         so you have n hordes of monsters 
         ith h contains ai monsters right ? 
         u have two types of attacks and combo counters right ? 
         there is two types of attack , right  ? 

         type 1 : 
         choose i such that atleast one monster in the ith is ai is alive then u kill on moster from that ith position ie ai-1 
         and increase x by 1

         type 2: 
         choose i , such that there is atleast x monster in the ith 
         ie ai>=x 
         then do ai-=x;
         x=0

         these are the types of operations  ? right  ? 

         what is the minimum number of attacks ,? how to find that 

         i think ye to easy hai 
         array ko sort karlenge 
         chhote elements ko peeto jab tk wo largest ke brabar na ho jaye
         phir largest ko bhi peet do 

         the thing is how to implement it ? 


  -> Your Attacks:
  so threer is minimum no of attacks , right ? to win the game as soon as possible ? right ? 
