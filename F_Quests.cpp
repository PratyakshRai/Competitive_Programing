...
/*
  -> Small Observations:
         agar number of coind kisi ek bhi array ki value se chhota hua 
         to infinite answer hoga, right ? 
         and if maxvalue of array * days chhota hua no of coin required se to bhi infinity 
         othewise we have to maximize the value of k 
         ham binary search karenge k  par 
         ham d ko divide karenge k+1 se aur dekhenge ki kitne block ban rhe hai 
         let say x block ban rhe hai 
         to , x*((sum of largest(min(k+1,n)values)))
         + jo remainder bacha hai usko phir se largest values me 
         utilize kar lenge 
...
*/
bool can(vector<int>&a,int n, int mid, int d, int c){
  int block=d/(mid+1);
  int sum =0;
  for(int i=0;i<min(n,mid+1);i++)sum+=a[i];
  sum*=block;
  int rem=d%(mid+1);
  for(int i=0;i<min(n,rem);i++)sum+=a[i];
 return sum>=c;
}
void Chal_Ja_Plz() {
