...
/*
  -> Small Observations:
         jisko dene wala koi nhi bachhe ga wo eliminate ho jayega ek din , right ? 
...
void Chal_Ja_Plz() {
    ...
    vector<int>a(n+1);
    set<int>curr;
    vector<multiset<int>> deya(n+1), liya(n+1);
    for(int i=1;i<=n;i++){cin>>a[i];
    deya[i].insert(a[i]);
    liya[a[i]].insert(i);
    }
    for(int i=1;i<=n;i++){
      if(liya[i].empty())curr.insert(i);
    }
    int ans=1;
    while(!curr.empty()){
      set<int>currzero;
      while(!curr.empty()){
        ...
        curr.erase(it);
        for(auto i :deya[it]){
          auto temp=liya[i].find(it);
          liya[i].erase(it);
          if(liya[i].empty())currzero.insert(i);
        }
      }
      for(auto it :currzero)curr.insert(it);
      ans++;
    }
    cout<<ans+1<<endl;
