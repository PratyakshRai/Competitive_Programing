...
void Chal_Ja_Plz() {
    ...
    vector<int>p;
    for(int i=1;i<=k;i++)p.pb(i);
    int idx=0;
    if(n%k==0&&m%k!=0){
      for(int i=0;i<n;i++)for(int j=0;j<m;j++)a[i][j]=p[(idx++)%k];
    }else{
      for(int j=0;j<m;j++)for(int i=0;i<n;i++)a[i][j]=p[(idx++)%k];
    }
    if(n%k==0&&m%k==0){
      for(int i=0;i<n;i++){if(i&1){idx=0;for(int j=0;j<m;j++)a[i][j]=p[(idx++)%k];}
    else {
      idx=k-1;
      for(int j=0;j<m;j++){a[i][j] = p[((idx % k) + k) % k]; 
        idx--;}
    }}
    if(k%2!=0){
      for(int i=0;i<n;i+=2)for (int j = 0; j < m; j += k) {
                    swap(a[i][j], a[i][j + k / 2]);
                }
    }
    }
