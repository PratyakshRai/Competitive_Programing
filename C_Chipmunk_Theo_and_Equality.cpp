...
/*
  -> Small Observations:
         now the thing is we have to calculate the minimum no of operation 
         to convert all the numbers same , right ?

         one thing to be noticed , we cannot increase a number ,like we can do atmax by 1 , 
         like if it is 3 , then it can become 4 , not more than this 

         so we have to only  check for the minimum value , and minimum value +1 if it is odd , right ? 

         no we cannot reach all the numbers ,right ,
...
void Chal_Ja_Plz() {
   ...
   for(int i=0;i<reach.size();i++){
    ...
    else if (number==reach[i].ff){
      cnt++;stp+=reach[i].ss;
    }else if (number!=reach[i].ff){
     if(cnt==n) ans=min(ans,stp);
     stp=reach[i].ss;
     cnt=1;
     number=reach[i].ff;
    }
