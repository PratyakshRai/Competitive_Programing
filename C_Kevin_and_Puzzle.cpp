...
*/
int mmod=998244353;
    // int dp(int idx ,int lier, vector<int>&a,bool bagalwala){
    //   if(idx==a.size())return 1;
    //   int ans=0;
    //   if(bagalwala==true){
    //     if(lier!=a[idx])return 0;
    //     else return  dp(idx+1,lier,a,false)%mmod;
    //   }else {
    //     ans+=dp(idx+1,lier+1,a,true);
    //     if(a[idx] == lier)
    // ans += dp(idx+1, lier, a, false);
    //   }
    //   return ans%mmod;
...
void Chal_Ja_Plz() {
    ...
    for(int i=1;i<=n;i++){
      ...
      if(i>=2&&a[i]==a[i-2]+1)dp[i][0]+=dp[i-2][0];
      dp[i][0]%=MOD;
      dp[i][1]%=MOD;
    }
