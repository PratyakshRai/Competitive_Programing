...
/*
  -> Small Observations:
         so we have to options for a particular ith location if i+1 does not follow its rule 
         either to change ith value or i+1 value , right ? 

         agar ith value ko change kar rhe hai to 
         i+1 jitna karde to theek hoga , 
         agar i+1 ko change kar rhe hai to 
...
void Chal_Ja_Plz() {
    ...
    vi a(n); vi c(n);
    invec(a);invec(c);
    // total - maximum jo hame keep karna hai , right ? 
