...
/*
  -> Small Observations:
         what is the thing is to move left 
         like suppose their is a number and it has to move left 
         we should have r r r r r r l l l l , like 
         n'r  concanated with  m'l
         now the question breaks down to 
         we have a range like the acutal number and the poistiohn where 
         we have to send it to , right  
         lets we define 

          we can move the number from x to y 
          x<y 
          or y to x if we have 

          rrrrr llllll 
          like it seems we  can swap two postion x and y if we have this 


          jo log pahale se hi sorted hai unhe nhi chhedenge 
...
*/
int check (string &s, vector<int>&bd,int idx){
  int n =bd.size();
  if(idx<1||idx>=n)return  0 ; 
  if(bd[idx]&&s[idx]=='L'&&s[idx+1]=='R')return 1;
  return 0;
}
void Chal_Ja_Plz() {
