...
/*
  -> Small Observations:
              ...
              okay the turns are infinite 

              alice goes first 

first lets focus on the functin : 

                we have the alt summation 
                alice tries to maximize it , how can she ? 


                kya bob ko swap karna jaruri hai ? 
                like see the number of turns 

                10^100 
                alice will always do the reverse or anything 

                so it is optimal for bob to quit the match 

                similarly , if for alice if it is no profitable swaps 
                quit the match right away 
                right ? 

                how to get the most profitable swaps ? 

                what number we can choose ? 

                + + , + - ? 

                can we say first we consider n legth swap ? 
                then check for the max profit ? 

                it will take n^2 complexity  

                what should i do ? 
                is there any else greedy ? 

                there are two factors which  will affect the answer 

                fisrt is due to swap , send is due to length 

                if we swap + + - - , nothing will happens , but just the length will added 
                if we swap + -  , some more will be added ? right ? 

                jab same symbol par swap karenge to 
                abs(i-j) ka fark ayega 

                but if swap on different sym like + - swap 
                to abs(i-j) +(ai-aj) ka difference ayega ? 

                how to get the greedy thing? 
...
void Chal_Ja_Plz() {
    ...
    int delta=0;
    if(n>=1)delta=max(delta,(n%2==1)?n-1:n-2);
    int minl=INF;
    for(int i=1;i<=n;i++){
      if(i%2!=0){
        minl=min(minl,2*a[i]+i);
      }
      else {
        delta=max(delta,2*a[i]+i-minl);
      }
    }
    int mx=-INF;
    for(int i=1;i<=n;i++){
