...
/*
  -> Small Observations:
         there is some more obsercation needed for this problem right ? 
         something is missing , something is not right ? 
         what is that  ? 

         what could be the answer 
         let reverse engineer this 

         if we gift all guys , the answer is zero , right  ? 
         ab agar maximum se gift hatayeinge to ghata hai 
         isiliye minimum jo hai usse ek gift chhin lenge ,
         uske feild view me jo bhi hai , wo sab ek ek contibute karenge , right ? 

         independent bna diya bhai ne 
...
void Chal_Ja_Plz() {
    ...
    invec(a);
    vector<int>b(3*n);
    for(int i=0;i<3*n;i++)b[i]=a[i%n];
    vector<int>pre(3*n+1,0);
    for(int i=0;i<3*n;i++)pre[i+1]=pre[i]+b[i];
    int mx=0;
    for(int i=n;i<2*n;i++){
        int tot=pre[i+d+1]-pre[i-d];
        int sum=-tot+b[i]+2*d*b[i];
        if(sum>0){
            mx+=sum;
        }
    }
