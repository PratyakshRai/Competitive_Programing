...
/*
  -> Small Observations:
         i think we have to find the longest increasing subsequence 
         but the problem here is that in lis there can be multiple decreasing sequences , rigth ? 

         lets put dp aside  , right ? 

         i thing i can have greedy , right ? 
         like for a particular answer 
         where it is comming from 
         there will be definatily a range be choosen right ? 
         we have to thing about the left , and right , 
...
int n;
int memo[300010][2][2];
int dp(int idx, vector<int>&a,bool start,bool chose,int sum ){
  if(idx==n){
    if(start&&!chose)return sum;
    else return -INF;
  }
  ...
  if(sum<a[idx])adder++;
  if(sum>a[idx])adder--;
  int newrating =sum+adder;
  if(!start&&!chose){
    ans=max(ans,dp(idx+1,a,false,false,newrating));
    ans=max(ans,dp(idx+1,a,true,true ,sum));
  }
  if(start&&chose){
    ans=max(ans,dp(idx+1,a,true,true,sum));
    ans=max(ans,dp(idx+1,a,true,false,newrating));
  }
  if(start&&!chose){
    ans=max(ans,dp(idx+1,a,true,false,newrating));
  }
  return memo[idx][(int)start][(int)chose]=ans;
}
void Chal_Ja_Plz() {
    ...
    //     for (int k = 0; k < 2; k++)
    //         memo[i][j][k] = -INF;
    // int t =dp(0,a,false,false,0);
    // if(t==-INF)cout<<0<<endl;
    // else cout<<t<<endl;
    int FF=0,TT=-1e15,TF=-1e14;
    auto fun=[&](int sum , int val){
      if(sum<=-1e15)return sum ;
      if(sum<val)return sum+1;
      if(sum>val)return sum-1;
      return sum ;
    };
    for(int i=0;i<n;i++){
      int nFF=fun(FF,a[i]);
      int nTT=max(TT,FF);
      int nTF = max(fun(TT, a[i]), fun(TF, a[i]));
      FF=nFF;
      TT=nTT;
      TF=nTF;
    }
