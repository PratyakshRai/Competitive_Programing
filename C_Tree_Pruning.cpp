...
/*
  -> Small Observations:
         i can do one thing like do binary search , 
         i can see ,we can do binary search here , right ? 
         
         now the question is after removing 
        //  mid no of edges , can we make that tree as per the question contraints ? 

        how should i even do that ? that is a big question ? 

        see ek baat to clear hai 
        mujhe ye btana hai ki kya mai 
        ek tree construct kar sakta hu 
        with all the child at the equal distance
...
void Chal_Ja_Plz() {
    ...
    // lavde ab dekh yaha pe ye pta chal gya ki ek node kis interval me jinda rahegi to bass maxy no of interval hamne nikal liya to maje hi maje , right ? 
    vi freq(n+4,0);
    for(int i=1;i<=n;i++){
      freq[depth[i]]++;
      freq[mxd[i]+1]--;
