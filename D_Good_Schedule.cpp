...
/*
  -> Small Observations:
         so one this is clear 

         we will get some good segments 
         let say l,r 
         so we get r-l+1 segments , right? 
         ye to wo segments hai jo l se suru hote hai 
         aur i par khatam honge , where i <=r 
         right ? 
         now there might be some cases , like 
         we got another segments starting from x where x>i 
         and ending at y , where y<=r , right  ? 
         kyuki l aur r ke andar bad nhi ho satka to hame bass
         starting points dekhne hai 
         kya ho sakte hai starting points 
         if both are 1 1 , or non are one one , right  
         okay i will count that starting point cnt++
         and addd their index 
         and when i reach the bad one 
         what i ll do is r*cnt-sum+cnt ,right ? , and it done ,yup

         ab isko implement karne me maut aayegi 
         lets try
