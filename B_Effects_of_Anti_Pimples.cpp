...
/*
  -> Small Observations:
         okay we have give an array a 
         some indices will be choosen , and colored black 
         after that all the multiples of the choosen black indices will 
         be colored greed , 

         now the score will be max of all the ai colored no white 
         similarly pick some other black  
         there will be 2^n -1 ways to color black 
         n is very large  , we cant iterate it all 
         if we got a configuration  , we cant tell the max value 
         without iterting on selected indices 
         we will sort the array and check when the a[n-i] will be selected as the maximum element ,, and how many times ? 

         let say we have 19 14 19 9 
         sorted array will be 
         9 14 19 19 
         when 19 we be the maximum and how many times ? 
         19 has the index 1,3 so when 1,3 or the divisors of 1,3 will be there , 19 is the answer for the max value 
         we cant calculate it directly , but can using set thoery 
         ie total possible ways - (1,3 aur unke divisors na aaye ) right? 
         x-y let 
         so the answer is (x-y)*19)%mod`  
         x will be the 2^n -1 )%mod which is ((2^n)modMOD−1+MOD)modMOD 
