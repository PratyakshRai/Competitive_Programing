...
/*
  -> Small Observations:
         find the number of sucgh boudaries b and c 
         such that 
         l<=length between b and c <=r
         and there are exactly k distinct numbers  
...
*/
int f(vector<int>&a,int k, int n ,int l, int r){
    map<int,int>mp;
    int i=0,j=0;
    int result=0;
    while(j<n){
        ...
        result+=max(0LL,min(j-i+1,r)-l+1);
        j++;
    }
    return result ;
