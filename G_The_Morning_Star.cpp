...
/*
  -> Small Observations:
         okay for a particular point same x ho ya to same y ho 
         wo sare points aa jayenge ? right ? 
         phir 
         origin shifting means like if i have a point and i have to check for 
         the diagonals 
         then what i will do , i will change the origin w r t all the points 
         x y will be at zero 
         hence x y will become 0 0 
         and all the other points px py
         will become px-x , py-y
         and then we will check 
         either how many of them follows abs(X)==abs(Y)

         what either we can do save the diagonal elements 
         and for a particular point we have to check which diagonal it 
         belongs to 
...
void Chal_Ja_Plz() {
    ...
    for(int i=0;i<n;i++){
      int px=a[i].ff;
      int py=a[i].ss;
      if(mp.find(px)!=mp.end())ans+=mp[px]-1;
      if(mpp.find(py)!=mpp.end())ans+=mpp[py]-1;
      if(dia1.find(px-py)!=dia1.end())ans+=dia1[px-py]-1;
      if(dia2.find(px+py)!=dia2.end())ans+=dia2[px+py]-1;
