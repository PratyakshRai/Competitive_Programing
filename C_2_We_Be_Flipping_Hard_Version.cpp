...
/*
  -> Small Observations:
         last second positive ko choose karunga ,right ? 
         phir jab tak +ve aa rha hai okay ,then 
         if negative comes  pick the negative , phir jab tk negative 
         chal rha hai , okay otherwise agar positive aa gya to pick the positive  , right ? 
...
void Chal_Ja_Plz() {
    ...
    int n;cin>>n;
    vector<int>a(n),pre(n+2),suff(n+2);
    for(int i=0;i<n;i++){
      cin>>a[i];
    }
    pre[0]=abs(a[0]);
    ...
    for(int i=idx-1;i>=0;i--){
      if(a[i]>0){
        if(oppo==false){an.pb(i+1);oppo=!oppo;}

      }else {
        if(oppo==true){an.pb(i+1);oppo=!oppo;}
      }
