...
/*
  -> Small Observations:
         so there is some action figures  , right? 
         the ith figuere cost i coins and it available from day i to n 
          for each day you know whether you can visit the shop or not 

            also let say you are visiting the shop you can buy  any number of 
            action figures  , right?    if you buy more than one action figure 
            you will get the expensive one free   
            so he wants to buy exactly one 
            1st , 1 from second  1 from third 1 from fourth ...
            inshort he has to buy exactly one from each item 
            what would be the minimum amount he has to pay ? 


            what is the best approach ,

            like for a particular day , we will try to chooce 
            only two of them 
...
void Chal_Ja_Plz() {
    ...
    //   o.erase((first));
    //   int second=INF;
    //   if(!o.empty()){second=*o.rbegin();
    
    // } 
    //     if(!z.empty()){
    //       int f=*z.rbegin();
    //       if(f>second)
    //       while(z.size()!=0&&f>second){
    //         ans+=f;
    //         z.erase(f);
    //         if(z.size()!=0)f=*z.rbegin();
