...
void Chal_Ja_Plz() {
    ...
    int ans=0;
    for(int i=0;i<n;i++){
        int m1=min(m/a[i].ff,a[i].ss)*a[i].ff;
        int m2=0;
        int m3=0;
        int m4=0;
        int m5=0;
        if(i<n-1&&a[i+1].ff-1==a[i].ff){
            m2=min(m/a[i+1].ff,a[i+1].ss)*a[i+1].ff;
            int y=0;
            y=(m-a[i].ss*a[i].ff)/a[i+1].ff;
            y=min(y,a[i+1].ss);
            y=max(0LL,y);
            int x=0;
            x=(m-(a[i+1].ff)*y)/a[i].ff;
            x=min(a[i].ss,x);
            x=max(0LL,x);
            m3=a[i].ff*x+a[i+1].ff*y;

             m4=0;
            int t1=min(a[i].ss,m/a[i].ff);
            int rem=m-t1*a[i].ff;
            int t2=min(a[i+1].ss,rem/a[i+1].ff);
            rem-=(t2*a[i+1].ff);
            m4=(t1*a[i].ff+t2*a[i+1].ff);
            int swap=min({rem,t1,a[i+1].ss-t2});
            m4+=swap;
        }
