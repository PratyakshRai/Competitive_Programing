...
void solve() {
    ...
    vector<ll>prevstn(n+1,0);
    for(int i =0;i<n;i++){
      prevstn[i+1]=prevstn[i]+stones[i];
    }

    sort(stones.begin(),stones.end());
    vector<ll>presorted(n+1,0);
    for(int i=0;i<n;i++){
      presorted[i+1]=stones[i]+presorted[i];
    }
  int q;
    cin>>q;
    while(q--){
      int l, r, type;
      cin>>type>>l>>r;
      ll ans=type==1?prevstn[r]-prevstn[l-1]:presorted[r]-presorted[l-1];
      cout<<ans<<endl;
    }
