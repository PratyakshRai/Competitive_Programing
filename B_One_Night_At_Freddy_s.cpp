  ...
  */
  ...
  /*
    -> Small Observations:
          n , m, l are the positive integer given , right 
          there are m animatorics waiting to jump scare you 
          the night consists of l seconds , numbered 1 to l 
          
          the jth of the ani has danger level dj , initil all are set
          to 0  , and each second exactly one of the ani 
          danger level increases by one , right ? 

          there are n fixed times ai <=l
          you can flash the light to any one of them  , which will 
          set their danger level to 0 
          let y be the overall danger level , ie the maximum danger level 
          you will loose if this danger level increases , ie maximum danger level is greater than x  , rigth ? 
          find the minimum value for x , such that regardless the action of ani you can garenttee the overall danger level is less than or equal to x ,, right ?


          what would the ani wil do , like he will try to  minimize the 
          damage to him , right ? 
          when even there is light flash , he will try to fill the 
          array in such a way that the damage is less, right ? 
          but how ? that is the question 

          one hint is that , m*l<=1e5 and similary m*n<=1e5  

          so the case one is , agar  n , m se chhota hai 
          to us case me ham n+1 ani choose karenge, aur 
          unhi me average kar ke daal denge , right ?  
          but the thing is average kaise daalenge , right ? 
          agar average 0 se chhota hai to discard kardo ,kyuki 
          flash light to unpar maarhi denge sab , otherwise score me add 
          kardo , right ? 
  ...
  */
  ...
  void Chal_Ja_Plz() {
      ...
      int curr=n;
      int req=min(n+1,m);
      for(int i=0;i<req;i++)st.insert(0);
      for(int i=1;i<=l;i++){
        auto it =st.begin();
        int val=*it;
        st.erase(it);
  ...
  st.erase(it);
          curr--;
          idx++;
          int req=min(curr+1,m);
          while(st.size()<req)st.insert(0);
        }
      }
      cout<<*st.rbegin()<<endl;
  }
