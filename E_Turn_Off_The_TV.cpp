...
/*
  -> Small Observations:
  //spare tree practice ;*)
         mujhe har tv par iterate karke dekhna hai ki 
         
         kya is range ko delete karu , to 
         baki sare millke iske range ko cover kar denge ? 
         right ? 
         
         how should i do that ? 
         l and r is very large, rigth 
         dekho ek baato to clear hai 

         mere pass ek idea hai , ki mai 
         prefix sum bna lu ranges kar 
         aur ek ek karke range lu aur us range me agar minimum 1 hua to wo redundant nhi hoga , aur agar >1 hua to wo redundant hoga right ? 


         wtf coordinate compression ? 
...
void Chal_Ja_Plz() {
    ...
    int max_size = points.size() + 2; 
    vector<int> diff(max_size, 0);
    
    vector<int> comp_l(n), comp_r(n);
    for (int i = 0; i < n; i++) {
        comp_l[i] = get_compressed(l[i]);
        comp_r[i] = get_compressed(r[i]);
        
        diff[comp_l[i]]++;
        diff[get_compressed(r[i] + 1)]--; 
    }
    
    vector<int> pref(max_size, 0);
    ...
    int mxsz=lg[t]+1;
    vector<vector<int>>sp(mxsz,vector<int>(t+2));
    for(int i=0;i<pref.size();i++)sp[0][i]=pref[i];
    ...
    for(int i=0;i<n;i++){
        int len=comp_r[i]-comp_l[i]+1;
        int j=lg[len];
        if(min(sp[j][comp_l[i]],sp[j][comp_r[i]-(1<<j)+1])>1){
            cout<<i+1<<endl;
            return ;
        }
    }
