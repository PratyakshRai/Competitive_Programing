...
/*
  ...
  -> Small Observations:
         okay so three toroidal neighbour ko same color dena hai right ? 
         okay i will check for row and col , both 
         like 2 simultaneous row is avaulable or not ,right ?
...
void Chal_Ja_Plz() {
    ...
    int n,m,k;cin>>n>>m>>k;
    vector<int>a(k);invec(a);
    if(fun(a,n,m)||fun(a,m,n))YES;
    else NO;
}
