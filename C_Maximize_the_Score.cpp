...
/*
  -> Small Observations:
         // greedy se ho ya na ho  , dp se to hohi jayega 

         agar ham maan lo ith ko choose kar rhe hai ,to kya hoga 
         fist ya to choose kar , yato na kre , right ? 
         to first occurence se pahale + ye interval 
         right ? 
...
void Chal_Ja_Plz() {
    ...
    for(int i=1;i<=2*n;i++){
        if(f.find(a[i])!=f.end()){
            ...
            int distance=i-j+1;
            distance*=distance;
            dp[i]=max({dp[i-1]+1,dp[j-1]+distance});
        }else {f[a[i]]=i;dp[i]=dp[i-1]+1;}
    }
    cout<<dp[2*n]<<endl;
