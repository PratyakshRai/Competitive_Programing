...
void Chal_Ja_Plz() {
    ...
    vi pret(N+1);
    vi prewt(N+1);
    vector<pair<int,int>>cntwt(N+1);
    vector<int>worker(N+1);
    vector<int>tester(N+1);
    for(int i=0;i<N;i++)prew[i+1]=prew[i]+a[i];
    for(int i=0;i<N;i++)pret[i+1]=pret[i]+b[i];
    for(int i=0;i<N;i++)prewt[i+1]=prewt[i]+max(b[i],a[i]);
    int w=0,t=0;
    for(int i=0;i<N;i++){
        if(a[i]>b[i])w++;
        else t++;
    ...
    for(int i=0;i<N;i++){
        ...
        int idw=lower_bound(worker.begin(),worker.end(),ww+worker[i])-worker.begin();
        int idt=lower_bound(tester.begin(),tester.end(),tt+tester[i])-tester.begin();
        int idx=min(idt,idw);
        cot+=prewt[idx+1]-prewt[i+1];
        if(idx==idt){
             cot+=prew[N];
            cot-=prew[idx+1];
        }else {
