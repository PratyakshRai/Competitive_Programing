...
/*
  -> Small Observations:
         ...
         if a is non decreasing , the game ends immediately

         operations 

         choose x 
         adn positive integers y and z 
         x=yz and 1<y  , z<x 
         it means they are the divisiors of x ? 
         if x is prime , we cant do anything ? 
         like there is no such way for prime numbers ? 
         rigth ? 
         
         now write y and z in any order at the original 
         location of x 
          if no such possible moves , the games ends 
          after the game ends 
          if a is increasig order , then bob wins 
          
          means , kahi bhi increasing order 
          ban gya to bob jeet jayega ? rigth ? 
  ...
  -> Your Attacks:
          ...
          see an element , if it is prime we
          cant do anything 
          otherwise 

          let the element is not prime , 

          after prime factorization 

          let say 2^x3^y... bna 
          agar sirf ek hi factor hai 
          to bhi alice kuch nhi kar sakta 

          other wise alice  ai ko 3* (2^x . 3 ^ y-1 )
          likh dega 
          ab kyuki ye mentions hai ki hame chale 
          chalni hi padengi 
          to bob na chahte hue bhi 
          aage proceed krega 
          bob chahega ki wo 2*() ki form likhe 
          but ab kya fayda 

          hame pahale 10^6 tk ke number ka 
          prime factor nikal lena hai 
          phir jo log ka factors 1 aur numeber itself 
          chodke sare form me store karenge 
          agar ek se jade factor aya to khel khatam 
          wahi par alice jeet gya 
          other wise bob wins 
...
void primefactors(int x){
  int temp=x;
  for(int i=2;i*i<=x;i++){
    if(x%i==0&&i!=temp)b[temp].pb(i);
    while(x%i==0){
      x/=i;
    }

  }
  if(x>1&&x!=temp)b[temp].pb(x);
}
