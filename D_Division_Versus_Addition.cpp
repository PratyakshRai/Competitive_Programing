...
long long get_log2(long long x) {
...
void solve() {
    ...
    vector<long long> base_moves(n + 1, 0);
    ...
    for (int i = 1; i <= n; i++) {
        ...
        base_moves[i] = base_moves[i - 1] + get_log2(x);
        count_B[i] = count_B[i - 1];
        count_C[i] = count_C[i - 1];

        if (is_power_of_two(x)) {
        } else if (is_power_of_two(x - 1)) {
            count_B[i]++;
        } else {
            count_C[i]++;
        }
    ...
    while (q--) {
        ...
        long long range_base_moves = base_moves[r] - base_moves[l - 1];
        long long range_B = count_B[r] - count_B[l - 1];
        long long range_C = count_C[r] - count_C[l - 1];

        long long ans = range_base_moves + range_C + (range_B / 2);
