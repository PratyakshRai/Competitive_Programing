...
/*
  -> Small Observations:
        okay lets do bruteforce first 
        
        // are bhai length decide karne ke baad agar height nhi aa rhi hai like decimal me nhi aana chahiye, right 
        s ko todna padega right ? 
        like the factors of s , rigth  
        kya ham bol sakte hai ki jaha hamara a x b  me  x range se bahar hua waha ham palat denge ? 
        x1 x y1 
        x2 x y2 
        x3 x y3
        x4 x y4 
        isme ham add karte jayenge jb tk hamara xi x se bda nhi ho jata right ? 

        uske baad kya , ham ulta kar denge ? 
        but overlap ka kya wo kaise handle hoga ? 
        okay so there will be a point 
        jaha se chije bigadengi , right ? 
        mere ko upar se us point tk aur 
        neeche se us point tk  
        dekhna hai , ki kaise hoga, right ? 
        then thing is how should i avoid overlapping ,right ? 
        one more thing , the maximum is x*y 
        so we should keep a check on that , right ? 
        i can do lowerbound do find that x , right  ?
...
int s,q;cin>>s>>q;
vi divs;
for(int i=1;i*i<=s;i++){if(s%i==0){divs.pb(i);
...
vi d(g),e(g),f(g);
for(int i=0;i<g;i++){
    d[i]=s/divs[i];e[i]=divs[i]-(i==0?0LL:divs[i-1]);
    f[i]=(i==0?0LL:f[i-1])+e[i]*d[i];
}
while(q--){
int x,y;cin>>x>>y;
int kam=lower_bound(all(divs),x)-divs.begin();
int adkhikwala=upper_bound(all(divs),s/y)-divs.begin()-1;
int ans=0;
int m=min(kam-1,adkhikwala);
if(m>=0)ans+=divs[m]*y;
if(kam-1>=m+1)ans+=f[kam-1]-(m>=0?f[m]:0LL);
if(kam<g){int u=x-(kam==0?0LL:divs[kam-1]);
int v=min(y,d[kam]);
