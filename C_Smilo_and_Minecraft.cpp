...
/*
  -> Small Observations:
         so agar maine pahala explosion binna kisi loss ke kar liya  , 
         to phir life set hai , mai sare gold le sakta hum , 
         otherwise we havet to minimize the first explosion but how? 
         its difficult to impliment , right /
...
void ok(int &x1, int &y1, int &x2, int &y2, int n, int m){
    x1 = max(x1, 1LL);
    x1 = min(x1, n);
    x2 = max(x2, 1LL);
    x2 = min(x2, n);
...
void Chal_Ja_Plz(){
    ...
    for (int i = 1; i <= n; i++){
        for (int j = 1; j <= m; j++) pref[i][j] = pref[i][j - 1] + g[i][j];
    }
    for (int i = 1; i <= m; i++){
    ...
    for (int i = 1; i <= n; i++){
        for (int j = 1; j <= m; j++){
            ...
            ok(x1, y1, x2, y2, n, m);
            ans = min(ans, pref[x2][y2] - pref[x1 - 1][y2] - pref[x2][y1 - 1] + pref[x1 - 1][y1 - 1]);
        }
    }
