...
/*
  -> Small Observations:
         like ek index choose kro i , 
         then dusara different index choose kro j 
         dono index different hone chahiye , right ? 
         aur aj ko replace kardo ai^aj se 

         infinite number of operations can be done right ? 
         agar ham do baar same choose karte hai , 
         to effect cancel ho ya yega, right 

         what does this means , like 
         agar mujhe kisi number ax ko bx ke equal karna hai 
         to mai kuchh koi bhi number us array se xor kar sakta hu right ? 
         see 
         at the end the conclusion is 
         we have to check whether we are able to make the ai =bi from the original array , not the modified one because 
         we can alway get what we want , right ? 

         then for each index ai where ai != bi , 
         we have to check if we can make ai = bi 
         by choosing some indexes not i 
         such that ai^[ax] = bi , right  ? 
         in other word 
         we have to see whether we can make 
         bi^ai , after choosing some index other than i , righ  ? 

         how to do that ? 
         we cannot do n^2 ,right ?  even if we do , we will always get wrong , right ? 

         zaha sirf zero hoga , wo fix kardi 

         yaar toda socho question kis or ishara kar rha hai , right ? 

         agar dekha jaye , 
         to har ek element ke liye ham 20*n times iterate nhi kar sakte hai , right ? 
         to ye cheej to idhar hi hat gayi , right ? 
         to aur kya hai mere pass, tools me is question ko karne ke liye, ? some precalculation , but kya 

         socho agar ham har ek digit ke liye ye tay karde , ki kya wo zero / one , independent ho sakti hai  ? 
         agar han to good 
         other wise 
         us digit ke waha par 1 rakhne se future me kaun kaun se digit dependent aur independent honge , kuchh ayesa sooch sakte hai kya ? 
         aur kya hi hai mere pass soochne ko , right ? 
...
void Chal_Ja_Plz() {
    ...
    A.insert(xa);
    B.insert(xb);
    for(int i:a)A.insert(i^xa);
    for(int i:b)B.insert(i^xb);
    if(A==B)YES;
    else NO;
}
