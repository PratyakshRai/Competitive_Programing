...
*/
  void dalo(set<pair<int,pair<int,int>>>&st,vector<int>&tables,int &idx){
    while(tables[idx]==0)idx++;
    int x=idx+1-tables[idx];
    tables[idx]--;
    int y=idx-x;
    cout<<3*x+1<<" "<<3*y+1<<endl;
    st.insert({3*x+1+3*y+2,{3*x+1,3*y+2}});
    st.insert({3*x+2+3*y+1,{3*x+2,3*y+1}});
    st.insert({3*x+2+3*y+2+2,{3*x+2,3*y+2}});
...
void Chal_Ja_Plz() {
    ...
    vector<int>tables(n+5);
    for(int i=0;i<tables.size();i++)tables[i]=i+1;
    int idx=0;
    for(int type:a){
      while(tables[idx] == 0) idx++;
      if(type==1){
        int next_fresh_dist = 3 * idx + 2;
        if(!st.empty()&& st.begin()->first < next_fresh_dist){
          auto it =st.begin();
