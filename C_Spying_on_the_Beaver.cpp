...
/*
  -> Small Observations:
         so hame bass ye dekhna hai ki root node se kitne ayese raaste hai jinme hamme well mill sakta hai , right ? 
         isse minimum well mill jayega ,right ? 

         naah this is not only the way out , right ? 
...
*/
bool fun(int node, int parent, vector<vector<int>>& adj, vector<int>& cuts, vector<int>& visited, vector<int>& well) {
    visited[node] = true;
    bool hswl = (well[node] > 0);
    for (int child : adj[node]) {
        if (child != parent) {
            bool flag = fun(child, node, adj, cuts, visited, well);
            if (flag) {
                if (hswl) {
                    cuts.pb(child);
                } else {
