...
*/
    ...
    void Chal_Ja_Plz() {
     ...
     auto kadane=[&](int l, int r){
      int mx=0,mn=0,curmax=0,curmin=0;
      for(int i=l;i<=r;i++){
        curmax=max(0LL,curmax+a[i]);
        mx=max(mx,curmax);
        curmin=min(0LL,curmin+a[i]);
        mn=min(curmin,mn);
      }
     ...
     if(pos==-1){
      ...
      kadane(0,pos-1);
      kadane(pos+1,n-1);
      int minpre=0,maxpre=0,cur=0;
      for(int i=pos+1;i<n;i++){
        cur+=a[i];
        minpre=min(minpre,cur);
        maxpre=max(maxpre,cur);
      }
      int minsuff=0,maxsuff=0;cur=0;
      for(int i=pos-1;i>=0;i--){
        cur+=a[i];
        minsuff=min(minsuff,cur);
        maxsuff=max(maxsuff,cur);
      }
      int high=maxsuff+a[pos]+maxpre;
      int low=minsuff+a[pos]+minpre;
...
// void dp(vector<int>&a,int idx,int sum,set<int>&st){
//     st.insert(sum);
