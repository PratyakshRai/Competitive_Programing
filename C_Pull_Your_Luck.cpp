...
/*
  -> Small Observations:
         ham try karenge minimum force calculate karne ka taki 
         0 par arrow aa sake , uske baad ham dekhenge ki kya 
         wo 1 se p ke range me hai ki nhi 

         let maan ho hamne f force lagaya

         f + f-1 + f-2 + f-3 .....f-(f-1)
         yaha tak hoga ? rigth? 
         total f terms hai 
         to f*f - sum of first f-1 terms ? right ? 
         n*(n+1)/2 is the sum of first n natural number 
         so 
         f*f-(f-1)*f/2
         so it become 
        let x = f*f+f)/2 , right ?
         we have to find that integer x 
         where 
         1<=x<=p is also satisfied  , rigth 
         this function is always increasing after 0 
         one more thing , the x value should be equal to 
         number of steps required to reach 0 which is n-y=a ,where y is the current arrow ? right ? 
         do the binary search ? right ?

         but wait a sec 11 7 100 
         is case me to khel khatam hai 
         kya hame modulo laga dena chahiye ? kya usse monotonous barkarar 
         rah jayega ? 
         but isse to loop me bhi fas sakta hai 

        (f*f+f)/2 =a+n*k , right ? 
        how to solve this ? how to get to know about this ? 
        like if (x-a)%n==0 or not ? 

        note the series is like the common difference is in ap , 
        like 1 3 6 10 15 21 28 36 45 55 66 78...
        how should i tell a+n*k this is a term for the series 
        i cant iterate on full p value its 10^9 ,right ? 
