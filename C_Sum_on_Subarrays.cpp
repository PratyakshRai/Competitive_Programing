...
void Chal_Ja_Plz() {
    ...
    // Step 1: Find largest m such that m*(m+1)/2 <= k
    int m = 0;
    while ((m + 1) * (m + 2) / 2 <= k) {
        m++;
    }
    
    // Step 2: Calculate remaining subarrays needed
    int rem = k - m * (m + 1) / 2;
    
    vi a(n);
    
    // Step 3: Fill first 'm' elements with a small positive number (like 2)
    for (int i = 0; i < m; i++) {
        a[i] = 2;
    }
    
    // Step 4: The Adjustment Element
    if (m < n) {
        // Place a carefully crafted negative number to get exactly 'rem' more positive subarrays
        a[m] = -(2 * (m - rem) + 1);
        
        // Step 5: Fill the remaining array with a very large negative number
        for (int i = m + 1; i < n; i++) {
            a[i] = -1000;
        }
    }
