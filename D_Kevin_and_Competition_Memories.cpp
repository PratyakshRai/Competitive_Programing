...
/*
  -> Small Observations:
          like maine kya soocha hai , ki ham m pe iterate karenge , like k=1 to m , and then see how many of the constest are possible , m/k , and we will iterate all the contests , we will pick k problems such that aur hero will have minimum ranking , like less number , but how ? till now we have mlogm ,


the thing is for a particular constest agar us contest me wo sare saawal hai jinki rating hero se kam ya brabar hai , to waise hi uska ranking 1 hoga , but the problem here is if it contains some equal and greater elemens , there is no meaning of picking higher ratings with lower rating question util we didnt have lower ratings ones left ,kyuki raking to phir higher rating ke hisaab se aane lagegi , kyuki lower rating to higher rating wale bande solve karhi denge, right ? 
...
void Chal_Ja_Plz() {
    ...
    for(int i:b){
      if(i<=a[0])j.pb(0);
        ...
        int it=(c.end()-lower_bound(c.begin(),c.end(),i));
      j.pb(it);
      }
    }
    srt(j);
